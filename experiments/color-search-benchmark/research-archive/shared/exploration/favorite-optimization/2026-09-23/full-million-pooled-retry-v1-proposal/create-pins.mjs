...
for (const name of ['retry-plan.json', 'favorite-full-million-four-methods-pooled.json', 'create-pins.mjs']) files.add(path.join(directory, name));
const pins = [];
...
const manifest = { schemaVersion: 1, experiment: 'favorite-pooled-retry-source-pins', createdAt: new Date().toISOString(), fileOnly: true,
  ...
  tests: { log: '/tmp/favorite-pooled-integration-tests.log', adapter: 4, benchmark: 11, benchmarkAudit: 11, arrival: 19, arrivalAudit: 12 },
...
await writeFile(path.join(directory, 'source-config-plan-pins-v1.json'), JSON.stringify(manifest, null, 2), { flag: 'wx' });
console.log(JSON.stringify({ pinCount: pins.length, manifest: path.join(directory, 'source-config-plan-pins-v1.json') }));
