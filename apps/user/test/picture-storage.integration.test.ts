...
describe('Private immutable picture storage', () => {
  ...
  const asset: StoredPicture = {
    id: 'pic_storage',
    profileId: 'owner',
    storageBucket: 'picture-adapter',
    storageKey: 'owner/pic_storage.webp',
    mimeType: 'image/webp',
    width: 2,
    height: 3,
    fileSizeBytes: 5,
  ...
  it('preserves first stored bytes, keeps objects private, and retries deletion safely', async () => {
    await runtime.runPromise(
      Effect.flatMap(PictureObjects, (objects) => objects.put(asset, Buffer.from('first')))
    );
    await expect(
      runtime.runPromise(
        Effect.flatMap(PictureObjects, (objects) => objects.put(asset, Buffer.from('other')))
      )
    ...
    expect(
      (
        await fetch(
          `${tester.s3.config.endpoints.fromHost}/${asset.storageBucket}/${asset.storageKey}`
        )
      ).status
    ).toBe(403);
    ...
    await expect(
      client.send(new GetObjectCommand({ Bucket: asset.storageBucket, Key: asset.storageKey }))
