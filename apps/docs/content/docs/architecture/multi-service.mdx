...
┌───▼────┐   ┌────▼─────┐   ┌───▼────┐   ┌───▼──────┐
│        │   │          │   │        │   │          │
│PostGres│   │ SeaweedFS │   │  NATS  │   │OpenSearch│
│        │   │          │   │        │   │          │
└────────┘   └──────────┘   └────────┘   └──────────┘
```
...
- Dependencies: PostgreSQL, SeaweedFS, NATS, Redis
...
- Dependencies: PostgreSQL, SeaweedFS, Redis
...
- Dependencies: SeaweedFS, NATS, FFmpeg
...
- Dependencies: SeaweedFS, NATS, PostgreSQL
- Events Consumed: `wallpaper.uploaded`
...
**Exports:**
- Connection managers (Database, SeaweedFS, NATS, Redis, OTEL)
...
- Infrastructure builders (Postgres, SeaweedFS, NATS, Redis, Docker)
- Fixture generators
- Test helpers
...
```typescript
import { DatabaseConnection, S3Connection, NatsConnectionManager } from '@wallpaperdb/core/connections';
import { container } from 'tsyringe';

// Register connections
container.registerSingleton(DatabaseConnection);
container.registerSingleton(S3Connection);
...
const s3 = container.resolve(S3Connection);
await s3.initialize();
...
Each application owns its Effect-based publication and consumption adapters. Shared event schemas validate public contracts; adapters translate them into local capability inputs. See [Events](/docs/packages/events) for delivery and replay requirements.
