import { pgTable, text, bigint, integer, timestamp, pgEnum, index, uniqueIndex, decimal } from 'drizzle-orm/pg-core';
...
export const wallpapers = pgTable('wallpapers', {
  ...
  contentHashUnique: uniqueIndex('idx_wallpapers_content_hash')
    ...
    .where(sql`${table.contentHash} IS NOT NULL AND ${table.uploadState} IN ('stored', 'processing', 'completed')`),
}));
