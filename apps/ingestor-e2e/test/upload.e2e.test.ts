...
describe('Upload E2E', () => {
	...
	test('upload JPEG wallpaper creates S3 object and database record', async () => {
		...
		const s3Objects = await tester.minio.listObjects(tester.minio.config.buckets[0]);
		...
		expect(['stored', 'processing', 'completed']).toContain(dbResult[0].upload_state);
	...
	test('upload without userId returns 400 error', async () => {
		...
		expect(body.type).toMatch(/missing-(user-id|file)/);
