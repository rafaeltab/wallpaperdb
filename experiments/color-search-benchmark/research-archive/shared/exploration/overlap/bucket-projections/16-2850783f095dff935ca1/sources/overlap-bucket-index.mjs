...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  for (const bucketCount of counts) {
    ...
    results.push({ bucketCount, index: result.index, count: result.count, completeIdsVerified: result.completeIdsVerified, projectionValuesVerified: result.projectionValuesVerified ?? bucketCount === 1024, reused: result.reused, directory: result.directory, projectionHash: result.projectionHash, indexMs: result.indexMs, storeBytes: result.stats?.primaries?.store?.size_in_bytes });
  }
