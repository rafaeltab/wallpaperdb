...
The User service publishes complete public Profile snapshots so consumers can build their own
version-aware projections without reading the command database.

- `ProfileCreatedEventSchema` uses the `profile.created` subject and records a typed `created`
  change.
- `ProfileUpdatedEventSchema` uses the `profile.updated` subject. Display-name updates record the
  previous and authoritative new value in a `display-name-changed` change.
- Handle changes record `handle-changed` with before/after Handles and any `scheduledAliases`,
  including exact expiry timestamps. Explicit scheduling records `alias-expiry-scheduled` with
  the Handle and before/after expiry values.
- Both contracts include the stable event ID, event timestamp, Profile version, Handle claim
  generation, and the complete post-change public snapshot. They contain no Clerk-private fields.
- Alias snapshots carry their creation time and nullable expiry time. These fields and the alias
  list remain optional when parsing retained events written before alias lifetime support.
...
const event: ProfileUpdatedEvent = ProfileUpdatedEventSchema.parse({
  eventId: "evt_01HABCDEF123456789",
