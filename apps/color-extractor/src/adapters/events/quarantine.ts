...
function metadata(message: JsMsg, reason: string, id: string, type: string): MsgHdrs {
  ...
  value.set('ce-source', 'https://wallpaperdb/color-extractor');
  value.set('ce-id', id);
...
const publish = Effect.fn('colors.quarantine.store')(function* (
  service: NatsBroker,
  data: Uint8Array,
  value: MsgHdrs
) {
...
function chunkMetadata(
  ...
  const value = metadata(
    ...
    reason,
    `${id}:${size}:${index}`,
    'color-extractor.upload.quarantine-chunk'
  );
...
export const quarantine = Effect.fn('colors.quarantine.publish')(function* (
  service: NatsBroker,
  message: JsMsg,
  reason: string
) {
  ...
  const single = metadata(message, reason, id, 'color-extractor.upload.quarantined');
