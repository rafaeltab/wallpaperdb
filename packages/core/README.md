...
- **Lifecycle-managed connections** — consistent `initialize / close / checkHealth` interface for PostgreSQL (via Drizzle ORM), S3-compatible object storage (SeaweedFS locally), NATS JetStream, Redis, OpenSearch, and OpenTelemetry; services extend the base connection classes to add TSyringe DI decorators and inject their own Drizzle schema
