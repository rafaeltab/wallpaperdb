...
class NatsHelpers {
    ...
    async getConnection(): Promise<NatsConnection> {
        if (!this.connection) {
            this.connection = await connect({
                servers: this.config.endpoints.fromHost,
            });
        }
    ...
    async publishEvent(subject: string, data: unknown): Promise<void> {
        ...
        js.publish(subject, JSON.stringify(data));
    }
        ...
        return class Nats extends Base {
            ...
            withNats(configure: (nats: NatsBuilder) => NatsBuilder = (a) => a) {
                ...
                this.addSetupHook(async () => {
                    ...
                    const ip = started.getContainer().getIpAddress("bridge");

                    const endpoints = {
                        networked: started.getConnectionUrl(networkAlias, 4222),
                        fromHost: started.getConnectionUrl(),
                        fromHostDockerInternal: started.getConnectionUrl(
                            "host.docker.internal",
                        ),
                        directIp: started.getConnectionUrl(ip, 4222),
                    };
