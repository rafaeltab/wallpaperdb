...
function fixture(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'ingestor-architecture-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const ingestor = path.join(root, 'apps/ingestor');
  for (const directory of ['scripts', 'packages', 'apps/ingestor/src', 'apps/ingestor/test']) {
    fs.mkdirSync(path.join(root, directory), { recursive: true });
  }
  fs.copyFileSync(path.join(repository, 'scripts/ingestor-architecture.mjs'), path.join(root, 'scripts/ingestor-architecture.mjs'));
  fs.writeFileSync(path.join(ingestor, 'package.json'), JSON.stringify({ type: 'module' }));
  fs.writeFileSync(path.join(ingestor, 'quality.config.json'), JSON.stringify({
    capabilities: ['catalogue', 'projection'],
    publicModules: ['catalogue', 'projection', 'adapters/search'],
  }));
  fs.symlinkSync(path.join(repository, 'apps/ingestor/node_modules'), path.join(ingestor, 'node_modules'), 'dir');
  ...
  return {
    root,
    write,
    source: (relative, content) => write(`apps/ingestor/src/${relative}`, content),
    run(script) {
...
function validGraph(project) {
  ...
  project.write('apps/ingestor/test/catalogue.test.ts', "import { search } from '../src/catalogue/index.js';\nsearch('value');\n");
}

test('architecture accepts public capability calls, private collaborators and adapter technology', (t) => {
  const project = fixture(t);
  validGraph(project);
  const result = project.run('ingestor-architecture.mjs');
...
test('architecture rejects private implementation imports from external tests', (t) => {
  const project = fixture(t);
  validGraph(project);
  project.write('apps/ingestor/test/catalogue.test.ts', "import { normalize } from '../src/catalogue/private.js';\n");
  const result = project.run('ingestor-architecture.mjs');
  assert.equal(result.status, 1);
...
test('architecture rejects container injection even in adapters', (t) => {
  ...
  const result = project.run('ingestor-architecture.mjs');
  assert.equal(result.status, 1);
  assert.match(result.stderr, /ingestor uses Effect service layers; remove tsyringe/);
});

for (const [name, directory, specifier] of [
  ['application', 'apps/other', '../../ingestor/src/catalogue/index.js'],
  ['shared package', 'packages/other', '@wallpaperdb/ingestor/catalogue'],
]) {
  test(`architecture rejects ${name} bypassing the deployed ingestor contract`, (t) => {
    const project = fixture(t);
    validGraph(project);
    project.write(`${directory}/src/index.ts`, `import { search } from '${specifier}';\n`);
    const result = project.run('ingestor-architecture.mjs');
    assert.equal(result.status, 1);
    assert.match(result.stderr, /another workspace must communicate with the ingestor through its external contracts/);
  });
}
