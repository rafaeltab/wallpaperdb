...
// - tester.minio (SeaweedFS S3 client)
...
| `MinioTesterBuilder` | S3/SeaweedFS storage | S3 client, bucket management |
...
SeaweedFS/S3 storage with bucket management. The public `MinioTesterBuilder`, `.withMinio()`, and `tester.minio` names remain for compatibility. Tests start `chrislusf/seaweedfs:4.47` and use the AWS SDK S3 client.
...
**Methods:**
- `withMinio(options?)` - Start SeaweedFS container
