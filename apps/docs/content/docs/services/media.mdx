...
- Event consumer for `wallpaper.uploaded` (caching metadata)
- Variant storage (pre-generated sizes)

### Planned 📋
- Format conversion (JPEG → WebP, etc.)
- CDN integration
- Image optimization (compression, quality tuning)
...
// MinIO download stream → Sharp transform → HTTP response
...
```
Client → CDN → Media Service → MinIO
         ↓
    (cache miss)
```
