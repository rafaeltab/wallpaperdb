...
The Ingestor Service is the entry point for wallpapers into WallpaperDB. It handles file uploads, validation, storage in SeaweedFS, metadata recording in PostgreSQL, and event publishing for downstream processing.
...
2. **uploading** - File being uploaded to SeaweedFS
   - Content hash calculated (SHA256)
   - File streamed to SeaweedFS
   - Metadata extracted (dimensions, MIME type, file size)

3. **stored** - File in SeaweedFS, metadata in DB, awaiting NATS publish
   - File successfully stored in SeaweedFS
...
6. State: uploading → Upload to SeaweedFS
...
#### 4. Orphaned SeaweedFS Reconciliation
Deletes SeaweedFS objects without DB records.
...
**Run tests:**
```bash
make test PACKAGE=ingestor          # All tests
make run PACKAGE=ingestor SCRIPT=test:watch    # Watch mode
```
...
Build and run development containers through Compose:

```bash
make apps-build SERVICE=ingestor  # Build image
make apps-start SERVICE=ingestor    # Run container
make apps-logs SERVICE=ingestor   # View logs
make apps-stop SERVICE=ingestor   # Stop container
```

### Production Build

```bash
make build PACKAGE=ingestor  # TypeScript compilation
make run PACKAGE=ingestor SCRIPT=start  # Run production build
```
...
│   │   ├── storage.service.ts              # SeaweedFS operations
