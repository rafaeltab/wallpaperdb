...
### User feedback — 2026-09-21

After inspecting the data, the user says the fine coverage-only histogram seems less promising than expected and that the coverage/quality combination is most likely better. They are considering a much larger vocabulary of coverage/quality regions, allowing one pixel to contribute to many regions, and ask whether a prototype already exists. This is a tentative preference between representations, not a new judged image ranking or a final method choice.

The existing named-feature extraction already accumulates coverage and conditional quality independently for every matching region, so overlap is present. Its vocabulary has 23 named color/vibe features, including two global distribution features. At that point the requested version with hundreds or thousands of independently stored coverage/quality pairs had not been built. The subsequent request to make one is now completed at [the overlapping-region inspector](http://zerotwo:8227/); [its findings](OVERLAPPING-REGIONS.md) cover 1,024 separate pairs, feedback and million-record measurements. The 4,913-anchor picked-color grid does accumulate overlapping area and quality at indexing time, but stores their combined precision utility rather than separate measurements; it is not the requested representation.

Preserve existing image judgments unchanged. Further bin count, region placement, query composition and scaling choices remain open.
