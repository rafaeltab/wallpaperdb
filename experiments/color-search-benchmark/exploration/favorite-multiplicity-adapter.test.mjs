...
import { createCandidate, METHODS } from './favorite-multiplicity-adapter.mjs';
...
test('adapter keeps aliases and parent semantics while declaring the fetch refinement source graph', async () => {
  ...
  for (const method of METHODS) {
    ...
    assert.equal(candidate.metadata.parentMethod, 'favorite-utility-numeric-docvalues');
    assert.equal(candidate.metadata.searchKind, 'favorite-multiplicity');
    assert.ok(candidate.metadata.sourceFiles.includes('exploration/adapter.mjs'));
    assert.ok(candidate.metadata.sourceFiles.includes('exploration/favorite-multiplicity-adapter.mjs'));
    assert.ok(candidate.metadata.sourceFiles.includes('exploration/favorite-multiplicity-utilities.mjs'));
  ...
  for (const method of METHODS) {
    ...
    const candidate = await createCandidate({ config: { id: 'alias', method: method.id, index,
      ...
      request: async (route, options) => {
        ...
        assert.equal(options.body.query.bool.should.length, 1);
        assert.equal(options.body.query.bool.should[0].function_score.field_value_factor.factor, 1);
