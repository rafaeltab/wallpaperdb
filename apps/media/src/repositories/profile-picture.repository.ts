...
import { profilePictureAssets, profilePictureHeads, type ProfilePictureAsset } from '../db/schema.js';
...
export class ProfilePictureRepository {
  ...
  async project(event: ProfileUpdatedEvent): Promise<void> {
    await this.database.getClient().db.transaction(async (tx) => {
      ...
      await tx.insert(profilePictureHeads).values(head).onConflictDoUpdate({
  ...
  async findCurrent(pictureId: string): Promise<ProfilePictureAsset | null> {
    const [result] = await this.database.getClient().db.select({ asset: profilePictureAssets })
      .from(profilePictureAssets)
      .innerJoin(profilePictureHeads, and(
