...
const started = performance.now(), shade = await prepared('shades'), original = await prepared('cutoffs');
...
let comparedValues = 0, differingValues = 0, maximumQualityDifference = 0;
const examples = [];
...
const result = { generatedAt: new Date().toISOString(), count: shade.assets.size,
  shadeIdentity: shade.metadata.identityHash, originalIdentity: original.metadata.identityHash,
  anchors: anchors.map(({ index, hex, lab }) => ({ index, hex, lab })),
  comparedValues, differingValues, maximumQualityDifference, exactParity: differingValues === 0,
  allSourceAndDescriptorHashesVerified: true, examples, wallMs: performance.now() - started,
};
await writeFile(path.join(shade.directory, 'neutral-parity-audit.json'), JSON.stringify(result, null, 2));
console.log(JSON.stringify(result, null, 2));
if (!result.exactParity) process.exitCode = 1;
