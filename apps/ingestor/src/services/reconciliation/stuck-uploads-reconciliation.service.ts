...
export class StuckUploadsReconciliation extends BaseReconciliation<WallpaperRecord> {
  ...
  protected async getRecordsToProcess(tx: TransactionType): Promise<WallpaperRecord[]> {
    const thresholdDate = new Date(Date.now() - ReconciliationConstants.STUCK_UPLOAD_THRESHOLD_MS);

    const records = await tx
      .select()
      .from(wallpapers)
      .where(
        and(eq(wallpapers.uploadState, 'uploading'), lt(wallpapers.stateChangedAt, thresholdDate))
      )
      .limit(1)
  ...
  protected async processRecord(record: WallpaperRecord, tx: TransactionType): Promise<void> {
    ...
    if (fileExists) {
      ...
      if (record.uploadAttempts >= ReconciliationConstants.MAX_UPLOAD_RETRIES) {
        ...
        console.log(`Marked upload ${record.id} as failed (max retries exceeded)`);
      } else {
