...
3. Starts infrastructure services (PostgreSQL, SeaweedFS, NATS, Redis, Grafana, OpenSearch, etc.)
4. Creates the `wallpapers` and `example-bucket` buckets in SeaweedFS
...
- Grafana: http://localhost:3000
- SeaweedFS S3 API: http://localhost:8002 (slot 0)

The S3 endpoint uses `8002 + 10 × slot` for other worktrees; inspect `S3_API_HOST_PORT` in `infra/.env`. Apps in Docker use `http://seaweedfs:9000`. For an existing environment, follow the [S3 data migration procedure](/docs/infrastructure/seaweedfs#existing-minio-data) before relying on stored wallpapers.
...
# Watch mode
make run PACKAGE=ingestor SCRIPT=test:watch

# Storage compatibility (real SeaweedFS container)
make storage-test
