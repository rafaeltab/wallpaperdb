...
const input: ExtractionInput = {wallpaperId:'wallpaper-test', fileType:'image', storage:{bucket:'wallpapers',key:'test.png'}, occurrence:{source:'https://wallpaperdb/ingestor',id:'uploaded-test'}, timestamp:'2026-09-24T10:00:00.000Z', correlationId:'workflow-test'};
...
it('awaits durable publication and preserves occurrence identity on replay', async () => {
 ...
 try {
  const publish = Effect.flatMap(ColorEvents, events => events.publish({input,histogram:[1,0],colorSpace:'hsv'}));
  ...
  const message = await manager.streams.getMessage('WALLPAPER',{last_by_subj:'wallpaper.colors.extracted'});
  expect(message.json()).toMatchObject({specversion:'1.0',type:'wallpaper.colors.extracted',time:input.timestamp,correlationid:'workflow-test',causationid:input.occurrence.id,data:{wallpaperId:input.wallpaperId,colorHistogram:[1,0],colorSpace:'hsv'}});
