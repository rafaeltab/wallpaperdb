...
export async function createOverlapInspectorProvider() {
  ...
  return {
    ...
    regionColors: args => args.metric === 'shade-hue-aware' ? hueRegionColors(args) : args.metric === 'shade-aware' ? shadeRegionColors(args) : args.profile ? cutoffRegionColors(args) : overlapRegionColors(args),
  };
}
