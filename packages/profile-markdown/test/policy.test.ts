...
describe("Profile Markdown policy", () => {
  ...
  it("collects unique Wallpaper IDs from inline and reference images for ownership validation", () => {
    ...
    expect(validateProfileMarkdown(source)).toEqual({
      valid: true,
      wallpaperIds: ["wlpr_ABC", "legacy-photo_2"],
    });
  });
  it.each([
    ...
    "[x](wallpaper:wlpr_one)",
    "[x](ftp://example.com)",
    "[x](https:example.com)",
    "[x](https://@example.com)",
    "[x](<https://example.com/a&#10;b>)",
    "[x](<https://example.com/&#92;evil>)",
    "<http://example.com>",
    "http://example.com",
    "person@example.com",
  ...
  it("normalizes external HTTPS destinations and exposes the actual hostname", () => {
    expect(normalizeProfileLink("HTTPS://BÜCHER.Example:443/a/../photo?q=1#view")).toEqual({
      href: "https://xn--bcher-kva.example/photo?q=1#view",
      hostname: "xn--bcher-kva.example",
    });
    expect(validateProfileMarkdown("[Gallery](https://example.com/photos)")).toEqual({
      valid: true,
      wallpaperIds: [],
    });
  });
  it("counts Unicode code points and enforces the configurable authored-source limit", () => {
    ...
    expect(validateProfileMarkdown("😀é", { maxCharacters: 2 })).toEqual({
      valid: true,
      wallpaperIds: [],
    });
    expect(validateProfileMarkdown("😀éa", { maxCharacters: 2 })).toEqual({
      valid: false,
      errors: [{ code: "too-long", message: "Biography must be 2 characters or fewer." }],
    });
