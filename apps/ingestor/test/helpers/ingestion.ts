...
export class ControlledStore implements IngestionStore {
  ...
  private owns(record: UploadRecord) { const current = this.records.get(record.wallpaper.id); return current?.state === record.state && current.leaseToken === record.leaseToken; }
  reserve: IngestionStore['reserve'] = (record, leaseUntil) => Effect.sync(() => {
  ...
  stored: IngestionStore['stored'] = (record) => Effect.sync(() => {
    ...
    this.deadlines.set(record.wallpaper.id, { changed: this.now(), lease: this.deadlines.get(record.wallpaper.id)?.lease ?? 0 });
  ...
  defer: IngestionStore['defer'] = (record, now, maxAttempts) => Effect.sync(() => {
  ...
  claim: IngestionStore['claim'] = (now, staleBefore, leaseUntil, limit) => Effect.sync(() => {
    const records = [...this.records.values()].filter((record) => {
      const deadline = this.deadlines.get(record.wallpaper.id);
      return (record.state === 'uploading' || record.state === 'stored') && record.attempts < (record.state === 'uploading' ? 3 : 10) && Boolean(deadline && deadline.changed < staleBefore.getTime() && deadline.lease <= now.getTime());
    }).slice(0, limit);
    return records.map((record) => { const claimed = { ...record, leaseToken: `${record.leaseToken}-claimed` }; this.records.set(record.wallpaper.id, claimed); this.deadlines.set(record.wallpaper.id, { changed: this.now(), lease: leaseUntil.getTime() }); return claimed; });
  });
...
export function fixture(overrides: { inspection?: ContentInspection; storage?: Partial<AssetStorage>; events?: UploadEvents } = {}) {
  ...
  const clock: Clock.Clock = { currentTimeMillisUnsafe: () => now, currentTimeMillis: Effect.sync(() => now), currentTimeNanosUnsafe: () => BigInt(now) * 1_000_000n, currentTimeNanos: Effect.sync(() => BigInt(now) * 1_000_000n), monotonicTimeNanosUnsafe: () => BigInt(now) * 1_000_000n, monotonicTimeNanos: Effect.sync(() => BigInt(now) * 1_000_000n), sleep: () => Effect.void };
  ...
  return { store, objects, published, layer, advance: () => Effect.sync(() => { now += 11 * 60_000; }) };
}
