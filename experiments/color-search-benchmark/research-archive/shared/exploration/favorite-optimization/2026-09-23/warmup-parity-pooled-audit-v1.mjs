...
export const METHODS = [baseline, 'favorite-utility-sorted-docvalues', 'favorite-utility-bounded-pooled-delete', 'favorite-utility-maxima-bounded-pooled-delete'];
const BINDINGS = {
  ...
  'favorite-utility-bounded-pooled-delete': ['./favorite-pooled-utilities.mjs','buildFavoritePooledQuery','executeFavoritePooledUtilitySearch'],
  'favorite-utility-maxima-bounded-pooled-delete': ['./favorite-pooled-utilities.mjs','buildFavoritePooledQuery','executeFavoritePooledUtilitySearch'],
};
const SHAPES = [
...
export function configuration(args) {
  ...
  for(let i=0;i<args.length;i++) { const key=args[i],value=args[++i]; assert.ok(['--directory','--receipt','--audit','--output','--count','--scope','--methods','--selectivities','--pins','--overlap'].includes(key)&&value&&!value.startsWith('--')); values[key.slice(2)]=value; }
  for(const key of ['directory','receipt','audit','output','pins','overlap']) assert.ok(values[key], 'Provide --'+key);
  ...
  return {...values,count,methods,selectivities,...Object.fromEntries(['directory','receipt','audit','output','pins','overlap'].map(k=>[k,path.resolve(values[k])]))};
}
...
export function verifyPooledWitness(evidence, method) {
  assert.ok(['favorite-utility-bounded-pooled-delete','favorite-utility-maxima-bounded-pooled-delete'].includes(method));
  assert.equal(evidence?.method,method);assert.equal(evidence.parentMethod,method.replace('-pooled-delete',''));
  const witness=evidence.transport;
  assert.equal(witness?.kind,'favorite-pooled-pit-delete');assert.equal(witness.version,1);
  for(const key of ['nativeDeleteRequests','nativeDeleteResponses','attempts'])assert.equal(witness[key],1);
  assert.ok([0,1].includes(witness.reusedConnections));
}
const PIN_MANIFEST_SHA256='d620d618f1f4ff9efba5188c79ce2a6c0a81ca9bea0c10c67fa290480fd7dc5f';
export function verifyPinnedSnapshot({pins,sources,plan,campaign,configurationBytes}) {
  assert.equal(pins.experiment,'favorite-pooled-retry-source-pins');assert.equal(pins.pinCount,44);assert.equal(pins.pins.length,44);
  const byPath=new Map(pins.pins.map(pin=>[pin.path,pin]));assert.equal(byPath.size,44);
  assert.equal(plan.configHash,campaign.configHash);assert.equal(plan.configHash,hash(configurationBytes));
  const configPin=byPath.get(plan.configuration.configFile);assert.ok(configPin);assert.equal(configPin.sha256,plan.configHash);
  assert.equal(configPin.bytes,configurationBytes.length);
  assert.ok(same(JSON.parse(configurationBytes).candidates,plan.candidates));
  for(const [file,text] of Object.entries(sources)) {
    const pin=byPath.get(path.join(pins.workspace,'experiments/color-search-benchmark',file));
    assert.ok(pin,'Archived source absent from approved pins: '+file);assert.equal(pin.sha256,hash(text));assert.equal(pin.bytes,Buffer.byteLength(text));
  }
  return {manifestSha256:PIN_MANIFEST_SHA256,configSha256:plan.configHash,archivedSourcesBound:Object.keys(sources).length};
}
export async function auditWarmupParity(config) {
  ...
  const result={schemaVersion:1,experiment:'favorite-pooled-warmup-cross-method-parity',fileOnly:true,auditedAt:new Date().toISOString(),configuration:config,rows,errors,warnings,
  ...
  try {
    ...
    const pins=await read('pins',config.pins);assert.equal(files.pins.sha256,PIN_MANIFEST_SHA256);
    const configurationBytes=await readFile(plan.configuration.configFile);files.config={path:plan.configuration.configFile,sha256:hash(configurationBytes)};
    result.approvedSourceBinding=verifyPinnedSnapshot({pins,sources,plan,campaign,configurationBytes});
    const overlap=await read('knownOverlap',config.overlap);
    assert.equal(overlap.startedAt,'2026-09-23T18:47:38.419Z');assert.equal(overlap.finishedAt,'2026-09-23T18:47:40.739Z');assert.equal(overlap.durationMs,2320);
    const overlapGroups=new Map();let rowsWithTimestamps=0,rowsMissingTimestamps=0,rowsMalformedTimestamps=0;
    ...
    assert.equal(receipt.uuid,'LHjoEvSiR8KNe5WZv_a7iA');
    ...
    for await(const line of createInterface({input:stream,crlfDelay:Infinity})) {
      // Bind the recorded host overlap in this same stream; no second scan.
      const started=/"startedAt"\s*:\s*"([^"]+)"/.exec(line)?.[1],finished=/"completedAt"\s*:\s*"([^"]+)"/.exec(line)?.[1];
      if(started&&finished&&Number.isFinite(Date.parse(started))&&Number.isFinite(Date.parse(finished))&&Date.parse(started)<=Date.parse(finished)){rowsWithTimestamps++;if(Date.parse(started)<=Date.parse(overlap.finishedAt)&&Date.parse(finished)>=Date.parse(overlap.startedAt)){
        const request=JSON.parse(line),group=overlapGroups.get(request.profileId)??{profileId:request.profileId,candidateId:request.candidateId,caseId:request.caseId,phase:request.phase,concurrency:request.concurrency,requestCount:0,errors:0,overOneSecond:0,firstStartedAt:started,lastCompletedAt:finished};
        group.requestCount++;group.errors+=Number(Boolean(request.error));group.overOneSecond+=Number(request.elapsedMs>=1000);
        if(started<group.firstStartedAt)group.firstStartedAt=started;if(finished>group.lastCompletedAt)group.lastCompletedAt=finished;
        overlapGroups.set(request.profileId,group);
      }}else { rowsMissingTimestamps++;if(started&&finished)rowsMalformedTimestamps++; }
    ...
    const afterStat=await stat(requests);assert.equal(beforeStat.ino,afterStat.ino);assert.equal(beforeStat.dev,afterStat.dev);assert.equal(beforeStat.size,afterStat.size);assert.equal(beforeStat.mtimeMs,afterStat.mtimeMs);
    files.requests={path:requests,sha256:digest.digest('hex'),bytes:afterStat.size};
    result.knownHostOverlap={...overlap,rowsWithTimestamps,rowsMissingTimestamps,rowsMalformedTimestamps,attributionComplete:rowsMissingTimestamps===0,affectedRequestGroups:[...overlapGroups.values()],
      profileResourceEnvelopes:(campaign.profiles??[]).filter(profile=>profile.before?.at<=overlap.finishedAt&&profile.after?.at>=overlap.startedAt)
        .map(profile=>({profileId:profile.id,candidateId:profile.candidateId,concurrency:profile.concurrency,beforeAt:profile.before.at,afterAt:profile.after.at})),
      interpretation:'Timestamp association only; overlapping rows are retained unchanged. This does not estimate or remove the replay impact.'};
    if(rowsMissingTimestamps)warnings.push('Some raw rows lack timestamps; overlap attribution is incomplete.');
    ...
    for(const item of expected) {
      ...
      if(!row.error&&item.method.includes('bounded')) {
        ...
        verifyPooledWitness(evidence,item.method);
      }
    }
