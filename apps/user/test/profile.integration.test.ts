...
describe('Profile commands', () => {
  ...
  async function expireAlias(userId: string, handle: string, expectedVersion: number) {
    ...
    return app.inject({
      method: 'POST',
      url: `/profile/me/aliases/${encodeURIComponent(handle)}/expire`,
      headers: { authorization: `Bearer ${token}` },
      payload: { expectedVersion },
    });
  }
  ...
  it('stores authored Biography Markdown and returns a complete authoritative versioned snapshot', async () => {
    ...
    const response = await app.inject({
      ...
      payload: { biographyMarkdown: authored, expectedVersion: original.version },
    });
    expect(response.statusCode).toBe(200);
    const updated = response.json();
    expect(updated).toMatchObject({
    ...
    expect(reactivated.aliases).toEqual([
      {
        ...
        createdAt: reactivated.updatedAt,
        expiresAt: null,
      },
    ]);
  ...
  it('keeps an expiring alias with the same claim and defeats its stale expiry candidate', async () => {
    ...
    try {
      ...
      expect((await reactivateAlias('user_1', original.handle, scheduled.version)).statusCode).toBe(
        409
      );
  ...
  it('uses the configured evidence window for historical Handle eligibility even while events remain unpublished', async () => {
    ...
    try {
      ...
      vi.setSystemTime(new Date('2030-01-08T00:00:00.000Z'));
      expect((await request('user_1')).json().historicalHandles).toEqual([]);
      expect((await reactivateAlias('user_1', original.handle, released.version)).statusCode).toBe(
        400
      );
      expect(
  ...
  it('renews an old alias only when a typed overflow schedule records that Handle', async () => {
    ...
    try {
      ...
      expect((await reactivateAlias('user_1', original.handle, renamed.version)).statusCode).toBe(
        400
      );
      ...
      expect(
        restored
          .json()
          .aliases.filter((alias: { expiresAt: string | null }) => alias.expiresAt === null)
  ...
  it.each([
    ...
    const before = released
      ? (await expireAlias('user_1', original.handle, scheduled.version)).json()
      : scheduled;
    const claimsBefore =
    ...
    try {
      expect((await reactivateAlias('user_1', original.handle, before.version)).statusCode).toBe(
        500
      );
      expect((await request('user_1')).json()).toEqual(before);
      expect(
  ...
  it('requires authentication and valid versioned commands to reactivate aliases', async () => {
    ...
    expect(
      (await app.inject({ method: 'PUT', url, payload: { expectedVersion: scheduled.version } }))
        .statusCode
    ).toBe(401);
    ...
    const currentHandle = await reactivateAlias(
      'user_1',
      changed.handle.toUpperCase(),
      scheduled.version
    );
  ...
  it('releases a due alias at its exact expiry and permits a newer claim generation', async () => {
    ...
    try {
      ...
      expect((await request('user_1')).json()).toEqual({
        ...expired,
  ...
  it('only permits authenticated owners to immediately expire scheduled aliases at the last-seen version', async () => {
    ...
    const unauthenticated = await app.inject({
      method: 'POST',
      url: `/profile/me/aliases/${original.handle}/expire`,
      payload: { expectedVersion: retained.version },
    });
  ...
  it('rolls back failed expiry events and retries them without blocking later due aliases', async () => {
    ...
    try {
      ...
      try {
        expect((await expireAlias(bad.id, bad.aliases[0].handle, bad.version)).statusCode).toBe(
          500
        );
        expect((await request(bad.id)).json()).toEqual(bad);
        expect(
          (await changeHandle(other.id, bad.aliases[0].handle, other.version)).statusCode
        ).toBe(409);
        vi.setSystemTime(new Date(bad.aliases[0].expiresAt));
        await maintenance((service) => service.expireAliases(new Date()));
        expect((await request(bad.id)).json()).toEqual(bad);
        expect((await request(good.id)).json()).toMatchObject({
          aliases: [],
      ...
      await maintenance((service) => service.expireAliases(new Date()));
      expect((await request(bad.id)).json()).toMatchObject({
        aliases: [],
  ...
  it('safely races workers, immediate expiry, and a new Handle claimant', async () => {
    ...
    try {
      ...
      expect((await request('user_1')).json()).toMatchObject({
        aliases: [],
        version: scheduled.version + 1,
      });
      expect(
  ...
  it('ignores an old expiry scan after the owner promotes and retains a newer claim for that Handle', async () => {
    ...
    try {
      ...
      const changedAgain = (
        await changeHandle('user_1', 'second-current', promoted.json().version)
      ).json();
  ...
  it('requires authentication, ownership, and the last-seen version to schedule an alias', async () => {
    ...
    const unauthenticated = await app.inject({
      method: 'DELETE',
      url: `/profile/me/aliases/${before.handle}`,
  ...
  it('serializes scheduling against a Handle change at capacity with coherent owner reads', async () => {
    ...
    try {
      ...
      expect(
        accepted.aliases.filter((alias: { expiresAt: string | null }) => alias.expiresAt !== null)
      ).toHaveLength(1);
      for (const read of reads)
        expect(read.json()).toEqual(read.json().version === owner.version ? owner : accepted);
      expect((await request('user_1')).json()).toEqual(accepted);
      expect(
        await sql`select * from outbox_events where payload->'profile'->>'version' = ${String(accepted.version)}`
      ).toHaveLength(1);
    } finally {
      vi.useRealTimers();
    }
  ...
  it('changes a Handle atomically and preserves the former Handle as an alias', async () => {
    ...
    expect(events[0].payload.profile.claimGeneration).toBeGreaterThan(
      response.json().aliases[0].claimGeneration
    );
  ...
  it('requires authentication and a positive expected version for Handle commands', async () => {
    ...
    const unauthenticated = await app.inject({
      method: 'PUT',
      url: '/profile/me/handle',
      payload: { handle: 'new', expectedVersion: 1 },
    });
    expect(unauthenticated.statusCode).toBe(401);
    const token = Buffer.from(JSON.stringify({ id: 'user_1' })).toString('base64');
    for (const payload of [
      {},
      { handle: 2, expectedVersion: 1 },
      { handle: 'new' },
      { handle: 'new', expectedVersion: 0 },
      { handle: 'new', expectedVersion: 1.5 },
    ]) {
      const response = await app.inject({
        method: 'PUT',
        url: '/profile/me/handle',
