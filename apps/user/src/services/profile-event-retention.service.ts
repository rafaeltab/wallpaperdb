...
export class ProfileEventRetentionService {
  ...
  async cleanupExpired(now: Date, isStopping = () => false): Promise<{ deleted: number; failed: number }> {
    ...
    const events = await db.select({ id: outboxEvents.id, createdAt: outboxEvents.createdAt }).from(outboxEvents)
      .where(and(eligible, this.cursor ? or(
        gt(outboxEvents.createdAt, this.cursor.createdAt),
        and(eq(outboxEvents.createdAt, this.cursor.createdAt), gt(outboxEvents.id, this.cursor.id))
      ) : undefined)).orderBy(outboxEvents.createdAt, outboxEvents.id).limit(100);
    let deleted = 0;
    let failed = 0;
