...
Reusable Testcontainers setups for integration testing. Provides custom NATS container configuration. S3 storage setup runs SeaweedFS through `MinioTesterBuilder` in `@wallpaperdb/test-utils`.
...
Composable test infrastructure builders using the TesterBuilder pattern. Provides 9 infrastructure builders (Docker, Postgres, SeaweedFS, NATS, Redis, Fixtures, Cleanup, Setup, Destroy) with type-safe composition.
