...
class MinioHelpers {
  ...
  get config(): MinioConfig {
    ...
    const config = (this.tester as any)._minioConfig;
    if (!config) {
   ...
   * ```
   */
  async uploadObject(bucket: string, key: string, body: Buffer | string): Promise<void> {
    await this.getS3Client().send(new PutObjectCommand({ Bucket: bucket, Key: key, Body: body }));
  }
  ...
  async objectExists(bucket: string, key: string): Promise<boolean> {
    try {
      ...
      // biome-ignore lint/suspicious/noExplicitAny: AWS SDK error types are complex
