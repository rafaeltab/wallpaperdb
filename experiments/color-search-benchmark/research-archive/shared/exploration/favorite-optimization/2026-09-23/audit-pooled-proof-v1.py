...
def audit_feedback():
 ...
 for c in run['candidates']:
  ...
  for case in c['cases']:
   ...
   assert p['samplesMs']==[r['elapsedMs'] for r in p['trials']];samples.extend(p['samplesMs']);assert all(not r.get('error') and r['elapsedMs']<1000 and len(r['hits'])==20 for r in p['trials'])
 ...
 report={'fileOnly':True,'reviewedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'verified':True,'runFile':str(runfile),'runSha256':filehash(runfile),'candidateSourceHashChecks':source_count,'candidates':proof,'auditorHash':filehash(pathlib.Path(__file__)),'limitations':['All32 saved accuracy calls per pooled candidate contain actual native transport witnesses. These limit1000 calls cover only545 assets and intentionally use the full-query fallback.','Each candidate has96 timed top20 samples. Timed feedback trials omit executor-stage traces; matching frozen runner/adapter source binds the same execution path, but timed cleanup witnesses cannot be reconstructed.','Human agreement is independently audited separately in pooled-feedback-independent-audit-v1.json. These timings are feedback integration diagnostics, not capacity evidence.']}
...
if __name__=='__main__': audit_fidelity();audit_feedback()
