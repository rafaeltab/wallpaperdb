// THROWAWAY PROTOTYPE: offline hue measurements in separate, provenance-bound indexes.
...
import { hueCoordinates, hueQualityForCoordinates, hueDefinition, hueIndexForCount } from './hue-definition.mjs';
import { api, finishIndex, safeIndexName, loadExpandedCorpus, STORE, hash } from './service.mjs';

export const HUE_STORE = path.join(STORE, 'hues');
export const HUE_SOURCE_FILES = Object.freeze(['hue-definition.mjs', 'hue-index.mjs', 'shade-definition.mjs', 'cutoff-definition.mjs', 'cutoff-blend.mjs', 'overlap-regions.mjs', 'overlap-banks.mjs', 'overlap-index.mjs', 'corpus-colors.mjs']);
export const HUE_MEASUREMENTS = Object.freeze(CUTOFF_LEVELS.map(({ cutoff }) => Object.freeze({ kernel: 'hard', cutoff })));
...
const anchorCount = OVERLAP_REGIONS.length, anchors = OVERLAP_REGIONS.map(region => hueCoordinates(region.lab));
const canonical = value => Array.isArray(value) ? value.map(canonical) : value && typeof value === 'object' ? Object.fromEntries(Object.keys(value).sort().map(key => [key, canonical(value[key])])) : value;
const equal = (left, right) => JSON.stringify(canonical(left)) === JSON.stringify(canonical(right));

export function createHueEncoder({ cacheLimit = 8192 } = {}) {
  if (!Number.isSafeInteger(cacheLimit) || cacheLimit < 0) throw Error('Invalid hue RGB cache limit.');
  const cache = new Map();
  ...
  function contributions(rgb) {
    ...
    rgbCalculations++;
    const pixel = hueCoordinates(rgbToLab([(rgb >>> 16) / 255, ((rgb >>> 8) & 255) / 255, (rgb & 255) / 255]));
    const indexes = [], qualities = [];
    for (let index = 0; index < anchorCount; index++) {
      const match = hueQualityForCoordinates(pixel, anchors[index]);
  ...
  return {
    encodePixels(rgba) {
      ...
      documents++;
      return { pixelCount, measurements: HUE_MEASUREMENTS.map((measurement, level) => ({ ...measurement,
...
export async function extractHue(input, { encoder = createHueEncoder() } = {}) {
...
function validateDescriptor(descriptor) {
  if (!Number.isInteger(descriptor?.pixelCount) || descriptor.pixelCount < 1 || descriptor.measurements?.length !== 5) throw Error('Expected complete measured hue descriptor.');
  for (let i = 0; i < 5; i++) {
    const data = descriptor.measurements[i];
    if (data.kernel !== 'hard' || data.cutoff !== CUTOFF_LEVELS[i].cutoff || data.coverage?.length !== anchorCount || data.quality?.length !== anchorCount) throw Error('Hue descriptor measurement layout differs.');
    if (!data.coverage.every(value => Number.isInteger(value) && value >= 0 && value <= 10000) || !data.quality.every(value => Number.isFinite(value) && value >= 0 && value <= 1)) throw Error('Invalid measured hue coverage or quality.');
    for (let j = 0; j < anchorCount; j++) {
      if (i && data.coverage[j] > descriptor.measurements[i - 1].coverage[j]) throw Error('Hue nested coverage is not monotonic.');
      if (data.coverage[j] > 0 && data.quality[j] + 1e-7 < data.cutoff) throw Error('Hue conditional quality falls below admission level.');
    }
  }
}
export function toHueDocument(feature, descriptor, bucketCount = 1024) {
...
export function hueMapping({ bucketCount = 1024, source = true, shards = 1 } = {}) {
...
async function preparationPlan() {
  ...
  const sourceHashes = Object.fromEntries(await Promise.all(HUE_SOURCE_FILES.map(async name => [name, hash(await readFile(new URL(name, import.meta.url)))])));
  const identity = { schemaVersion: 1, definition: hueDefinition(), measurements: HUE_MEASUREMENTS, anchorsHash: hash(OVERLAP_REGIONS), originalFingerprints, sourceHashes };
  const identityHash = hash(identity), directory = path.join(HUE_STORE, identityHash.slice(0, 20));
...
async function readCheckpoint(plan, asset, expectedHash) {
  ...
  if (expectedHash && hash(bytes) !== expectedHash) throw Error(`Hue checkpoint hash mismatch: ${asset.id}.`);
  const data = JSON.parse(bytes);
  if (data.id !== asset.id || data.sha256 !== asset.sha256 || data.identityHash !== plan.identityHash) throw Error(`Hue checkpoint provenance mismatch: ${asset.id}.`);
...
async function loadPrepared(plan) {
  ...
  if (metadata.identityHash !== plan.identityHash || !equal(metadata.identity, plan.identity) || metadata.count !== plan.corpus.length) throw Error('Hue preparation metadata mismatch.');
  if (!equal(metadata.assets.map(asset => asset.id).sort(), plan.corpus.map(asset => asset.id).sort()) || new Set(metadata.assets.map(asset => asset.id)).size !== plan.corpus.length) throw Error('Hue checkpoint manifest IDs differ from original corpus.');
  if (hash(metadata.assets) !== metadata.descriptorHash) throw Error('Hue checkpoint manifest fingerprint mismatch.');
  for (const [name, expectedHash] of Object.entries(plan.identity.sourceHashes)) if (hash(await readFile(path.join(plan.directory, 'sources', name))) !== expectedHash) throw Error(`Hue source snapshot changed: ${name}.`);
  return { ...plan, metadata };
}
async function originalDocuments() { return new Map((await loadOverlapDocuments()).map(document => [document.id, document])); }
async function prepareHueDescriptors() {
  ...
  if (originals.size !== plan.corpus.length || plan.corpus.some(asset => !originals.has(asset.id))) throw Error('Original measured corpus IDs differ from hue inputs.');
  ...
  for (const name of HUE_SOURCE_FILES) {
    const filename = path.join(plan.directory, 'sources', name);
    try { if (hash(await readFile(filename)) !== plan.identity.sourceHashes[name]) throw Error(`Hue existing source snapshot differs: ${name}.`); }
    catch (error) { if (error.code !== 'ENOENT') throw error; await copyFile(fileURLToPath(new URL(name, import.meta.url)), filename); }
  }
  const encoder = createHueEncoder(), assets = [], started = performance.now();
  let reused = 0;
  console.error(`Hue checkpoints: ${plan.directory}`);
  for (const asset of plan.corpus) {
    ...
    try { checkpoint = await readCheckpoint(plan, asset); reused++; }
      ...
      const descriptor = await extractHue(bytes, { encoder });
    ...
    if (assets.length % 10 === 0 || assets.length === plan.corpus.length) console.error(`Hue extraction ${assets.length}/${plan.corpus.length}; ${reused} reused; ${Math.round((performance.now() - started) / 1000)}s`);
  }
  ...
  await writeFile(path.join(HUE_STORE, 'current.json'), JSON.stringify({ directory: plan.directory, identityHash: plan.identityHash, count: metadata.count }, null, 2));
  return { ...plan, metadata };
}
async function projectedDocument(prepared, asset, originals, bucketCount) {
  ...
  return toHueDocument(original, descriptor, bucketCount);
}
export async function loadHueDocuments(bucketCount = 1024) {
...
function indexMetadata(prepared, bucketCount) {
  return { experiment: 'shade-hue-aware-cutoff-coverage-quality', schemaVersion: 1, metric: 'shade-hue-aware', bucketCount, count: prepared.metadata.count, identityHash: prepared.identityHash, descriptorHash: prepared.metadata.descriptorHash, sourceHashes: prepared.identity.sourceHashes, anchorsHash: hash(overlapRegionsForCount(bucketCount)), directory: prepared.directory };
}
async function verifyExisting({ prepared, originals, bucketCount, index, allowIncomplete = false, expectedIds }) {
  ...
  if (expectedIds && !equal([...expectedIds].sort(), expected)) throw Error('Hue verifier requires all original corpus IDs.');
  const mapping = (await api(index + '/_mapping')).body[index]?.mappings, metadata = indexMetadata(prepared, bucketCount);
  if (mapping?.dynamic !== 'strict' || mapping?._source?.enabled === false || !equal(mapping?.properties, hueMapping({ bucketCount }).mappings.properties) || !equal(mapping?._meta, metadata)) throw Error('Hue index mapping or provenance differs from measured inputs.');
  ...
  if (response.timed_out || response._shards?.failed || response.hits.total?.value !== ids.length || actualSet.size !== ids.length || ids.some(id => !expectedSet.has(id)) || (!allowIncomplete && !equal([...ids].sort(), expected))) throw Error('Hue index must contain exactly expected corpus IDs.');
  ...
  for (let offset = 0; offset < assets.length; offset += 5) {
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      if (!actual.found || actual._id !== document.id || !equal(actual._source, document)) throw Error(`Hue indexed values differ from measured source: ${document.id}.`);
...
export async function verifyHueIndex({ bucketCount = 1024, index = hueIndexForCount(bucketCount), expectedIds } = {}) {
...
export async function prepareHueIndexes({ counts = OVERLAP_BUCKET_COUNTS } = {}) {
  if (!counts.length || new Set(counts).size !== counts.length) throw Error('Expected unique hue bucket counts.');
  counts.forEach(count => overlapRegionsForCount(count));
  const prepared = await prepareHueDescriptors(), originals = await originalDocuments(), results = [];
  for (const bucketCount of counts) {
    const index = hueIndexForCount(bucketCount), started = performance.now();
    let present = true;
    try { await api(index + '/_mapping'); } catch (error) { if (/^OpenSearch 404:/.test(error.message)) present = false; else throw error; }
    if (!present) {
      const mapping = hueMapping({ bucketCount }); mapping.mappings._meta = indexMetadata(prepared, bucketCount);
    ...
    for (let offset = 0; offset < missing.length; offset++) {
      ...
      if (response.errors) throw Error(`Hue create-only bulk failed: ${JSON.stringify(response.items).slice(0, 1800)}`);
      if ((offset + 1) % 25 === 0) await api(index + '/_flush', { method: 'POST', timeoutMs: 300000 });
      if ((offset + 1) % 100 === 0 || offset + 1 === missing.length) console.error(`Hue ${bucketCount} indexing ${existingIds.size + offset + 1}/${prepared.corpus.length}`);
    }
    ...
    console.error(`Hue ${bucketCount} ready: ${result.count} wallpapers, ${result.stats.primaries.store.size_in_bytes} bytes.`);
  }
  return results;
}
export const buildHueIndexes = prepareHueIndexes;
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const results = args.includes('--verify') ? await (async () => { const out = []; for (const bucketCount of counts) out.push(await verifyHueIndex({ bucketCount })); return out; })() : await prepareHueIndexes({ counts });
