...
The S3 endpoint uses `8002 + 10 × slot` for other worktrees; inspect `S3_API_HOST_PORT` in `infra/.env`. Apps in Docker use `http://seaweedfs:9000`. For an existing environment, follow the [MinIO data migration procedure](/docs/infrastructure/seaweedfs#existing-minio-data) before relying on stored wallpapers.
