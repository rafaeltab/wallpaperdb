...
function runCi(target: string, failingStage: string) {
...
`, { mode: 0o700 });
    ...
    writeFileSync(log, '');
    const result = spawnSync('make', ['--no-print-directory', target, `TURBO=${join(directory, 'turbo')}`], {
      cwd: directory,
      encoding: 'utf8',
      timeout: 10_000,
      env: {
...
describe.each(['ci', 'ci-force'])('%s failure propagation', (target) => {
  it.each([
    ...
    const result = runCi(target, stage);
  ...
  it('reports success only after every stage succeeds', () => {
    const result = runCi(target, 'none');
