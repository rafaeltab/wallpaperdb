...
import { OVERLAP_METHODS, supportsOverlap, buildOverlapQuery, overlapParameters } from './methods-overlap.mjs';
import { overlapIndexForCount } from './overlap-banks.mjs';
import { CUTOFF_METHODS, supportsCutoff, buildCutoffQuery, cutoffParameters } from './methods-cutoff.mjs';
import { cutoffIndexForCount } from './cutoff-definition.mjs';
import { shadeIndexForCount } from './shade-definition.mjs';
import { hueIndexForCount } from './hue-definition.mjs';
import { FAVORITE_OPTIMIZED_METHODS, supportsFavoriteOptimized, buildFavoriteOptimizedQuery, favoriteOptimizedParameters } from './favorite-optimized-scoring.mjs';
import { FAVORITE_TYPED_METHODS, supportsFavoriteTyped, buildFavoriteTypedQuery } from './favorite-typed-scoring.mjs';
...
export const METHODS = Object.freeze([...BASE_METHODS, BOUNDED_METHOD, ...FAST_METHODS, TRANSPORT_METHOD, ...REFINEMENTS, ...DIRECT_PALETTE_METHODS, ...RANK_FEATURE_METHODS, RELATIVE_METHOD, ...WIDER_ANN,PALETTE_BOUNDED_METHOD,INDEXED_HYBRID,...PRECISION_TYPED_METHODS,INDEXED_TYPED_HYBRID,...PRECISION_PRECOMPUTED_METHODS,INDEXED_PRECOMPUTED_HYBRID,...NATIVE_REFINED_METHODS,CLICKHOUSE_METHOD,...PRECISION_GRID_METHODS,...OVERLAP_METHODS,...CUTOFF_METHODS,...FAVORITE_OPTIMIZED_METHODS,...FAVORITE_TYPED_METHODS,...FAVORITE_UTILITY_METHODS,...FAVORITE_PRECISION_METHODS,...FAVORITE_SORTED_METHODS,...FAVORITE_BOUNDED_METHODS]);
export function indexForMethod(input,index=INDEX,parameters={}){const method=getMethod(input);return index===INDEX?(['favorite-sorted-utilities','favorite-bounded-utilities'].includes(method.searchKind)?'color-exploration-favorite-points-real-v2':method.searchKind==='favorite-precision-utilities'?'color-exploration-favorite-precision-real-v1':method.searchKind==='favorite-utilities'?'color-exploration-favorite-opt-real-v4':['favorite-optimized','favorite-typed'].includes(method.searchKind)?hueIndexForCount(favoriteOptimizedParameters(parameters).bucketCount):method.searchKind==='cutoff'?(method.metric==='shade-hue-aware'?hueIndexForCount:method.metric==='shade-aware'?shadeIndexForCount:cutoffIndexForCount)(cutoffParameters(method,parameters).bucketCount):method.searchKind==='overlap'?overlapIndexForCount(overlapParameters(method,parameters).bucketCount):method.searchKind==='precision-grid'?PRECISION_GRID_INDEX:method.family==='rank-features'?RANK_FEATURE_INDEX:index):index;}
...
export function supports(input,query,options={}) {
  ...
  if(method.searchKind==='favorite-bounded-utilities')return supportsFavoriteBounded(method,query,options);
  if(method.searchKind==='favorite-sorted-utilities')return supportsFavoriteSorted(method,query,options);
  if(method.searchKind==='favorite-precision-utilities')return supportsFavoritePrecision(method,query,options);
  if(method.searchKind==='favorite-typed')return supportsFavoriteTyped(method,query,options);
  if(method.searchKind==='favorite-optimized')return supportsFavoriteOptimized(method,query,options);
  if(method.searchKind==='favorite-utilities')return supportsFavoriteUtilities(method,query,options);
  if(method.engine==='clickhouse')return supportsClickHouse(query,options);
  if(method.searchKind==='cutoff')return supportsCutoff(method,query,options);
  if(method.searchKind==='overlap')return supportsOverlap(method,query,options);
...
export async function executeSearch({index,seedIndex,table,method:input,query,limit=20,eligibleIds,excludedIds,filter,parameters={},signal,timeoutMs=10000,serviceTimeout}) {
  ...
  index=indexForMethod(method,index,parameters);
  if(method.searchKind==='favorite-bounded-utilities')return executeFavoriteBoundedUtilitySearch({index,method,query,limit,eligibleIds,excludedIds,filter,parameters,signal,timeoutMs,serviceTimeout});
  if(method.searchKind==='favorite-sorted-utilities')return searchFavoriteSortedUtilities({index,method,query,limit,eligibleIds,excludedIds,filter,parameters,signal,timeoutMs,serviceTimeout});
  ...
  const body=method.searchKind==='favorite-precision-utilities'?buildFavoritePrecisionQuery(request):method.searchKind==='favorite-typed'?buildFavoriteTypedQuery(request):method.searchKind==='favorite-optimized'?buildFavoriteOptimizedQuery(request):method.searchKind==='favorite-utilities'?buildFavoriteUtilityQuery(request):method.searchKind==='cutoff'?buildCutoffQuery(request):method.searchKind==='overlap'?buildOverlapQuery(request):method.searchKind==='precision-grid'?buildPrecisionGridQuery(request):method.searchKind==='native-refined'?buildNativeRefinedQuery(request):method.searchKind==='precision-precomputed'?buildPrecisionPrecomputedQuery(request):method.searchKind==='precision-typed'?buildPrecisionTypedQuery(request):method.searchKind==='relative'?buildRelativeQuery(request):method.family==='rank-features'?buildRankFeatureQuery(request):method.searchKind==='direct-palette'?buildDirectPaletteQuery(request):method.searchKind==='transport'?buildTransportQuery(request):method.baseMethod?buildFastQuery(request):buildQuery(request);
