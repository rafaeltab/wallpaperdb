...
- `S3Connection` - S3-compatible object storage
...
Applications own Effect-based publishers and consumers. The shared package exports contracts only. See [Events](/docs/packages/events) for validation, identity, acknowledgement, and quarantine requirements.
...
### @wallpaperdb/profile-markdown

Shared Biography parsing and validation for User-service commands and Web rendering. The package owns the supported CommonMark/GFM syntax, Unicode character counting, HTTPS destination normalization, and `wallpaper:` image references.

User applies the configured authoring limit and checks the returned wallpaper IDs against its local ownership projection. Web uses the same policy and remark plugin with React Markdown and sanitization, rendering wallpaper references through a dedicated component. Public rendering validates syntax without reapplying the current authoring limit, so a changed limit cannot hide already published Biographies.

Authored Markdown remains the stored and event-published representation. See the [shared Biography policy decision](/docs/architecture/decisions/profile-markdown) for the service responsibilities.
...
Custom Testcontainers implementations for NATS, PostgreSQL, SeaweedFS, and Redis.
