...
const health = availabilityLayer.pipe(
  Layer.provide(
    Layer.succeed(AvailabilityProbe, {
      inspect: () =>
        Effect.succeed({ database: true, s3: true, nats: true, otel: true, consumer: true }),
    })
  )
);
const delivery = Layer.succeed(MediaDelivery, {
  wallpaper: () => Effect.succeed({ _tag: 'NotFound' }),
  picture: () => Effect.succeed({ _tag: 'NotFound' }),
});
const services = Layer.merge(health, delivery);
...
it('serves health, readiness, OpenAPI, CORS and safe errors', async () => {
  const app = await createHttpApp({ nodeEnv: 'development', port: 3006 }, services);
  try {
    expect((await app.inject('/health')).json()).toMatchObject({
      status: 'healthy',
      checks: { database: true, s3: true, nats: true, otel: true, consumer: true },
    });
...
it.each([
  { contentType: 'application/json', payload: `{"secret":"${privateRequestData}"`, status: 400 },
  {
    ...
    payload: privateRequestData + 'x'.repeat(1024 * 1024 + 1),
    status: 413,
  },
  ...
  try {
    ...
    expect(response.json()).toEqual({
      ...
      type: expect.stringContaining('/invalid-request.md'),
    });
...
it('hides defects and releases scoped services on close', async () => {
  ...
  const app = await createHttpApp({ nodeEnv: 'test', port: 3006 }, Layer.merge(layer, delivery));
...
it('releases acquired services when graph startup fails', async () => {
  ...
  await expect(
    createHttpApp({ nodeEnv: 'test', port: 3006 }, Layer.merge(layer, delivery))
  ).rejects.toBeDefined();
  expect(closed).toBe(true);
});
