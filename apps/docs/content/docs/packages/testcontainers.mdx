...
Create an OpenSearch testcontainer with security and authentication:

```typescript
import { OpenSearchContainer } from '@wallpaperdb/testcontainers/containers';

describe("OpenSearch Integration", () => {
  let container: StartedOpenSearchContainer;

  beforeAll(async () => {
    container = await new OpenSearchContainer('opensearchproject/opensearch:2.11.1')
      .withSecurityEnabled(true)
      .withPassword('MyStr0ng!P@ssw0rd')
      .start();
  });
...
class OpenSearchContainer {
  /** Enable/disable OpenSearch security plugin (default: true) */
  withSecurityEnabled(enabled: boolean): this;

  /** Set admin password (must have zxcvbn score ≥ 3) */
...
```typescript
interface StartedOpenSearchContainer {
  /** Get HTTP(S) endpoint URL */
  getHttpUrl(from: 'host' | 'directIp'): string;

  /** Get mapped HTTP(S) port */
  getPort(from: 'host' | 'directIp'): number;

  /** Get URL schema (http or https) */
  getSchema(): string;
...
**Security Features:**
- Security plugin enabled by default with HTTPS
- Password strength validation using zxcvbn (score ≥ 3 required)
- Basic authentication with admin user
- Self-signed certificate for HTTPS (test environments)
...
S3 test storage is provided by `MinioTesterBuilder` in [`@wallpaperdb/test-utils`](/docs/packages/test-utils#miniotesterbuilder). It starts SeaweedFS and exposes bucket management and an AWS SDK S3 client. The historical builder name remains compatible with existing tests.
