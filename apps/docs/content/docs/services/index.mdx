...
- Search public Profiles with `searchProfiles(query, first, after)` and cursor pagination. Current Handles rank above aliases, followed by Display name phrases/prefixes and fuzzy names. Biography is excluded from search, and results do not enumerate aliases. Wallpaper documents retain ownership IDs; Profile identity stays in its separate index.
