...
def execution(root,result):
    ...
    for trace in lines(root/'service-traces.jsonl'):
        ...
        if trace['method']=='favorite-utility-bounded':
            ...
            assert all(t['request']['body']['pit']['id']==opened[0] for t in searches)
            final=searches[-1];ids=[h.get('fields',{}).get('id',[h['_id']])[0] for h in final['response']['body']['hits']['hits']]
...
def main():
    ...
    receipt=dict(schemaVersion=1,kind=args.kind,directory=str(args.directory),auditedAt=datetime.datetime.now(datetime.timezone.utc).isoformat(),passed=True,
