...
ck(r.get('finishedAt') and not r.get('interruptions'),'completed no interruption')
...
ck(not r['skipped'],'C1 no omitted stage')
...
ck(sum(rawcounts.values())+len(resources)==r['recording']['totalRows'],'recorder totalrows')
ck(rawbytes+resbytes==r['recording']['totalBytes'],'recorder totalbytes')
