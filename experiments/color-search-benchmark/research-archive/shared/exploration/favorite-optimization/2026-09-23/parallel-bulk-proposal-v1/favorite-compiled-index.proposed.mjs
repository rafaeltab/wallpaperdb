...
export function favoriteCompiledIndexConfiguration(args = []) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    const key = args[i];
    if (!['--mode', '--scope', '--count', '--index', '--directory', '--encodings', '--presets', '--base', '--source', '--samples', '--numeric-points', '--bulk-concurrency'].includes(key)) throw Error('Unknown utility indexing option: ' + key);
    const value = args[++i];
  ...
  return { mode, scope, count, index, directory, base: url.origin, encodings: chosen, presets,
    ...
    dryRun: values.dryRun ?? false, samples, bulkConcurrency, bulkBytes: 5 * 1024 * 1024 };
}
...
export async function runFavoriteCompiledIndex(config) {
  const prepared = await prepare(config), { inputs, plan, mapping, sourceSnapshot, source, identity, identityHash, encode, compileMs } = prepared;
  ...
  try {
    ...
    bulkScheduler = createFavoriteBulkScheduler({ concurrency: config.bulkConcurrency,
      ...
      onEvent: async event => {
        ...
        if (event.phase === 'accepted') {
          receipt.indexed += event.documents; receipt.bulkCount++; receipt.bulkBytes += event.bytes;
        ...
        receipt.bulkScheduler = bulkScheduler.snapshot();
        await save();
      },
    });
    for (let ordinal = 0; ordinal < config.count; ordinal++) {
      ...
      for (const batch of batcher.add(document)) await bulkScheduler.submit(batch);
    }
    for (const batch of batcher.finish()) await bulkScheduler.submit(batch);
    receipt.bulkScheduler = await bulkScheduler.finish();
    ...
    // Successful in-flight create acknowledgements remain counted and logged.
    if (bulkScheduler) receipt.bulkScheduler = await bulkScheduler.drain();
