...
def terminate(process):
    # A reaped leader does not imply its process group is gone. Always address
    # the group, including after the leader has already returned an exit code.
    if process is None:return
    signal_group(process.pid,signal.SIGTERM)
    grace=time.monotonic()+TERMINATION_GRACE_SECONDS
    while group_alive(process.pid) and time.monotonic()<grace:
        process.poll()  # Reap the leader when possible, without ignoring children.
        time.sleep(min(.02,max(0,grace-time.monotonic())))
    signal_group(process.pid,signal.SIGKILL)
    try:process.wait(timeout=2)
    except subprocess.TimeoutExpired:raise GateFailure('Process-group leader survived SIGKILL')
    ...
    def run(command,logname):
        ...
        with (directory/logname).open('xb') as out:
            ...
            try:code=active.wait(timeout=max(.01,deadline-time.monotonic()))
                ...
                if active.poll() is not None and not group_alive(active.pid):
                    active=None;state.pop('activeProcess',None)
        if active is not None:
            # An exited Make leader with remaining descendants is not a clean
            # phase boundary. Terminate the group and refuse to start more work.
            terminate(active);active=None;state.pop('activeProcess',None)
            raise GateFailure('Phase leader exited with a surviving process group: '+logname)
        return code
    try:
        validate_static(plan)
