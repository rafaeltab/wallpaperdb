...
function InlineField({ field, profile, tokenProvider }: Props) {
  ...
  async function save() {
    if (!edit || pending.current || phase === 'success' || queryClient.isFetching({ queryKey: key }) || queryClient.isMutating({ mutationKey: key })) return;
    ...
    } finally { if (live.current && phase !== 'success') pending.current = false; }
  }
  const locked = phase === 'saving' || phase === 'success';
  const actionLabel = phase === 'saving' ? `Saving ${label}` : phase === 'success' ? `${title} saved` : `Save ${label}`;
  ...
  return <div ref={container} className="min-w-0">
    <form className={`flex min-w-0 max-w-full items-baseline gap-1 ${typography}`} onSubmit={(event) => { event.preventDefault(); void save(); }}>
      {edit ? <>
        ...
        <ProfileActionButton label={actionLabel} type="submit" textBaseline buttonClassName="size-6" disabled={locked || refreshing || writing || edit.value === edit.baseValue}>
          {phase === 'saving' ? <Loader2 className="size-[1ex] animate-spin motion-reduce:animate-none" /> : <Check className="profile-save-feedback size-[1ex]" data-save-phase={phase} />}
        </ProfileActionButton>
