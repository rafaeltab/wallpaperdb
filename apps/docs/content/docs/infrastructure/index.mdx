...
Default values work out of the box for local development. Worktree setup assigns the S3 host port as `8002 + 10 × slot`. Existing environments must update storage variables and regenerate application environments; see [storage migration](/docs/infrastructure/seaweedfs#existing-minio-data).
...
Both allow anonymous object downloads for local development. Listing and writes require credentials. See [SeaweedFS](/docs/infrastructure/seaweedfs) for configuration and the migration procedure for existing S3 data.
