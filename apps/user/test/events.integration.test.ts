...
function wallpaperEvent(id: string) {
  return {
    eventId: id,
...
beforeEach(async () => {
  ...
  await manager.streams.purge('USER_QUARANTINE').catch((error) => {
...
it('publishes the original occurrence and leaves completion separate from acknowledged broker storage', async () => {
  ...
  try {
    ...
    await runtime.runPromise(
      Effect.flatMap(MaintenanceStore, (store) =>
        store.markPublished('event-publication', new Date(occurred))
      )
    );
    expect(
      await runtime.runPromise(Effect.flatMap(MaintenanceStore, (store) => store.pendingEvents()))
    ).toEqual([]);
  } finally {
...
it('publishes legacy creation and typed updates, carries persisted trace context, and ignores other subjects', async () => {
  ...
  const updated = {
    ...
    change: { type: 'display-name-changed', before: 'Owner', after: 'Changed' },
    profile: { ...created.profile, displayName: 'Changed', version: 2 },
  };
...
it('retains corrupt recorded events instead of publishing a different identity or owner', async () => {
  ...
  try {
    const result = await runtime.runPromise(
      Effect.flatMap(ProfileEvents, (events) => events.publish('bad')).pipe(Effect.result)
    );
...
it('expires only acknowledged Profile evidence at the exact cutoff and preserves current data and unacknowledged events', async () => {
  ...
  try {
    expect(
      await runtime.runPromise(
        Effect.flatMap(MaintenanceStore, (store) =>
          store.expiredEvents(new Date('2029-12-31T23:59:59.999Z'))
        )
      )
    ).toEqual([]);
    expect(
      await runtime.runPromise(
        Effect.flatMap(MaintenanceStore, (store) => store.expiredEvents(new Date(occurred)))
      )
    ).toEqual([{ id: 'old-acknowledged', createdAt: new Date(occurred) }]);
    for (const id of ['old-acknowledged', 'old-pending', 'new-acknowledged', 'unrelated']) {
      expect(
        await runtime.runPromise(
          Effect.flatMap(MaintenanceStore, (store) =>
            store.deleteExpiredEvent(id, new Date(occurred))
          )
        )
      ).toBe(id === 'old-acknowledged');
    }
    expect((await sql`select id from outbox_events order by id`).map((row) => row.id)).toEqual([
      'new-acknowledged',
      'old-pending',
      'unrelated',
    ]);
...
it('keeps the first recorded wallpaper owner across replay before Profile creation', async () => {
  const runtime = ManagedRuntime.make(adapters());
  try {
    const accept = (profileId: string) =>
      Effect.flatMap(MaintenanceStore, (store) =>
        store.recordWallpaperOwnership({ wallpaperId: 'wp_1', profileId })
      );
    await runtime.runPromise(accept('first-owner'));
    await Promise.all([
      runtime.runPromise(accept('second-owner')),
      runtime.runPromise(accept('first-owner')),
    ]);
    expect(await sql`select wallpaper_id, profile_id from wallpaper_ownership`).toEqual([
      { wallpaper_id: 'wp_1', profile_id: 'first-owner' },
    ]);
  } finally {
    await runtime.dispose();
  }
...
it('accepts validated ownership facts through a durable consumer and acknowledges only completed projection', async () => {
  const options = {
    url: nats.getConnectionUrl(),
    stream: 'WALLPAPER',
    serviceName: 'user-consumer-contract',
    retryDelayMs: 10,
      ...
      Layer.provide(
        Layer.succeed(Maintenance, {
          ...
          recordWallpaperOwnership: (ownership) =>
            Effect.sync(() => {
              accepted.push(ownership.wallpaperId);
            }),
        })
      )
    )
  ...
  try {
    expect(
      await runtime.runPromise(Effect.flatMap(ConsumerHealth, (health) => health.check()))
    ).toBe(true);
    await connection
      .jetstream()
      .publish('wallpaper.uploaded', JSON.stringify(wallpaperEvent('first')));
    await expect.poll(() => accepted).toEqual(['wp_first']);
    await expect
      .poll(
        async () =>
          (await manager.consumers.info('WALLPAPER', 'user-wallpaper-ownership')).num_ack_pending
      )
      .toBe(0);
    expect(
      (await manager.consumers.info('WALLPAPER', 'user-wallpaper-ownership')).config.max_deliver
    ).toBe(-1);
  } finally {
    await runtime.dispose();
...
it('quarantines invalid messages and exhausted projection failures after three delayed attempts', async () => {
    ...
    ownershipConsumerLayer(options).pipe(
      ...
      Layer.provide(
        Layer.succeed(Maintenance, {
          ...
          cleanupEvents: unused,
          expireAliases: unused,
          recordWallpaperOwnership: () =>
            Effect.suspend(() => {
              attempts.push(Date.now());
              return Effect.fail(
                new MaintenanceFailure({ operation: 'project', cause: 'offline' })
              );
            }),
        })
      )
  ...
  try {
    ...
    await connection
      .jetstream()
      .publish('wallpaper.uploaded', JSON.stringify(wallpaperEvent('exhausted')));
    ...
    await expect
      ...
      .toBe(0);
    const recorded = await manager.streams.getMessage('USER_QUARANTINE', {
      last_by_subj: 'user.quarantine',
    });
    ...
    expect(JSON.parse(Buffer.from(evidence.data, 'base64').toString())).toEqual(
      wallpaperEvent('exhausted')
    );
  } finally {
    await runtime.dispose();
    await manager.consumers.delete('WALLPAPER', 'user-wallpaper-ownership');
...
it('keeps quarantine failures unacknowledged and repairs purged partial chunks before accepting responsibility', async () => {
  const options = {
    url: nats.getConnectionUrl(),
    stream: 'WALLPAPER',
    serviceName: 'user-quarantine-contract',
    retryDelayMs: 20,
    shutdownTimeoutMs: 200,
  };
    ...
    ownershipConsumerLayer(options).pipe(
      ...
      Layer.provide(
        Layer.succeed(Maintenance, {
          ...
          cleanupEvents: unused,
          expireAliases: unused,
          recordWallpaperOwnership: unused,
        })
      )
    )
  );
  ...
  try {
    await runtime.runPromise(ConsumerHealth);
    await manager.streams.update('USER_QUARANTINE', {
      max_msgs: 1,
      max_msg_size: 1024,
    ...
    metadata.set('ce-source', 'https://original.example');
    await connection.jetstream().publish('wallpaper.uploaded', original, { headers: metadata });
    await expect
      .poll(async () => (await manager.streams.info('USER_QUARANTINE')).state.messages)
      .toBe(1);
    await expect
      .poll(() => runtime.runPromise(Effect.flatMap(ConsumerHealth, (health) => health.check())))
      .toBe(false);
    expect(
      (await manager.consumers.info('WALLPAPER', 'user-wallpaper-ownership')).num_ack_pending
    ).toBe(1);
    ...
    const manifest = JSON.parse(
      new TextDecoder().decode(
        (await manager.streams.getMessage('USER_QUARANTINE', { last_by_subj: 'user.quarantine' }))
          .data
      )
    );
    ...
    for (const sequence of manifest.sequences)
      chunks.push((await manager.streams.getMessage('USER_QUARANTINE', { seq: sequence })).data);
    ...
    await manager.consumers.delete('WALLPAPER', 'user-wallpaper-ownership');
    await manager.streams.purge('WALLPAPER');
    await manager.streams.update('USER_QUARANTINE', { max_msgs: -1, max_msg_size: -1 });
  }
});
...
it('drains an accepted ownership update and acknowledges it before closing the broker', async () => {
  const options = {
    url: nats.getConnectionUrl(),
    stream: 'WALLPAPER',
    serviceName: 'user-drain-contract',
  ...
  try {
    ...
    await connection
      .jetstream()
      .publish('wallpaper.uploaded', JSON.stringify(wallpaperEvent('drain')));
...
it('interrupts stalled ownership work at the drain deadline and leaves the event redeliverable', async () => {
  const options = {
    url: nats.getConnectionUrl(),
    stream: 'WALLPAPER',
    serviceName: 'user-interrupt-contract',
  ...
  try {
    ...
    await connection
      .jetstream()
      .publish('wallpaper.uploaded', JSON.stringify(wallpaperEvent('interrupted')));
    await Effect.runPromise(Deferred.await(started).pipe(Effect.timeout(2000)));
    await runtime.dispose();
    expect(interrupted).toBe(true);
    const consumer = await connection
      .jetstream()
      .consumers.get('WALLPAPER', 'user-wallpaper-ownership');
...
it('accepts structured and binary CloudEvents while quarantining contradictory or malformed envelopes', async () => {
  const options = {
    url: nats.getConnectionUrl(),
    stream: 'WALLPAPER',
    serviceName: 'user-envelope-contract',
    retryDelayMs: 10,
    shutdownTimeoutMs: 200,
  };
  const accepted: string[] = [];
  ...
  try {
    ...
    await js.publish('wallpaper.uploaded', JSON.stringify(wallpaperEvent('binary')), {
      headers: metadata,
    });
    await js.publish(
      'wallpaper.uploaded',
      JSON.stringify({
        ...
        data: { wallpaper: wallpaperEvent('structured').wallpaper },
      })
    );
    await js.publish('wallpaper.uploaded', JSON.stringify(wallpaperEvent('contradiction')), {
      headers: metadata,
    });
    await js.publish(
      'wallpaper.uploaded',
      JSON.stringify({
        ...wallpaperEvent('malformed-structured'),
        specversion: '1.0',
        source: 'https://wallpaperdb/ingestor',
...
it('requires envelope agreement on source and time and a validated wallpaper owner before projection', async () => {
  const options = {
    url: nats.getConnectionUrl(),
    stream: 'WALLPAPER',
    serviceName: 'user-envelope-agreement',
    retryDelayMs: 10,
    shutdownTimeoutMs: 200,
  };
  const accepted: Array<{ wallpaperId: string; profileId: string }> = [];
  const unused = () => Effect.die('Unexpected maintenance task');
  const runtime = ManagedRuntime.make(
    ownershipConsumerLayer(options).pipe(
      Layer.provide(brokerLayer(options)),
  ...
  try {
    ...
    metadata.set('ce-source', 'https://another-producer.example');
    await js.publish('wallpaper.uploaded', JSON.stringify(structured), { headers: metadata });
    metadata.set('ce-source', structured.source);
    metadata.set('ce-time', '2030-01-01T00:00:01.000Z');
    await js.publish('wallpaper.uploaded', JSON.stringify(wallpaperEvent('agreed')), {
      headers: metadata,
    });
    metadata.set('ce-time', occurred);
    metadata.set('ce-source', 'not-a-uri');
    await js.publish('wallpaper.uploaded', JSON.stringify(wallpaperEvent('agreed')), {
      headers: metadata,
    });
    const invalidOwner = {
      ...wallpaperEvent('missing-owner'),
    ...
    await expect
      ...
      .toBe(0);
    expect(accepted).toEqual([{ wallpaperId: 'wp_agreed', profileId: 'owner-before-profile' }]);
  } finally {
    await runtime.dispose();
    await manager.consumers.delete('WALLPAPER', 'user-wallpaper-ownership');
    await manager.streams.purge('WALLPAPER');
