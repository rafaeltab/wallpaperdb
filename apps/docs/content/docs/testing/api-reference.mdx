...
Manages SeaweedFS (S3-compatible) containers. The `MinioTesterBuilder` name is retained for compatibility; `.withMinio()` and `tester.minio` use the same S3 API as before.
