...
describe('Picture persistence adapter', () => {
  ...
  const run = <A>(operation: (store: PictureStore) => Effect.Effect<A, unknown>) =>
    runtime.runPromise(Effect.flatMap(PictureStore, operation));
  beforeAll(async () => {
    ...
    runtime = ManagedRuntime.make(
      pictureStoreLayer({ bucket: 'pictures' }).pipe(
  ...
  it('requires a finished PUT before a durable candidate becomes staged', async () => {
    ...
    expect(await run((store) => store.finishUpload(asset.id, new Date(now.getTime() + 1000)))).toBe(
      true
    );
    expect(await sql`select state, upload_lease_until from profile_picture_assets`).toEqual([
      { state: 'staged', upload_lease_until: null },
    ]);
  ...
  it('marks expired candidates deleting before storage work and never claims a live PUT', async () => {
    ...
    asset = {
      ...(await run((store) =>
        store.createCandidate({
          ...asset,
          createdAt: current,
          expiresAt: new Date(current.getTime() + 1000),
        })
      )),
      createdAt: current,
    };
    ...
    expect(await run((store) => store.claimDeletion(asset.id, cutoff))).toMatchObject({
      id: asset.id,
    });
  ...
  it('fences retry writes after a later replica replaces the import lease', async () => {
    ...
    const second = await run((store) =>
      store.claimImport('owner', new Date(now.getTime() + 1000), 1000)
    );
    ...
    expect(await sql`select source_url, lease_token from profile_picture_imports`).toEqual([
      { source_url: 'https://img.clerk.com/private', lease_token: second.leaseToken },
    ]);
    await run((store) => store.settleImport(second, true, now));
    expect(
      await sql`select source_url, status, lease_token, lease_until from profile_picture_imports`
    ).toEqual([{ source_url: null, status: 'complete', lease_token: null, lease_until: null }]);
  });
  ...
  it('preserves diagnostic SQLSTATE without exposing captured source credentials in failures or logs', async () => {
    ...
    await sql.unsafe(
      `create function reject_import_retry() returns trigger language plpgsql as $$ begin raise exception 'Rejected source %', NEW.source_url; end $$`
    );
    await sql.unsafe(
      `create trigger reject_import_retry before update on profile_picture_imports for each row execute function reject_import_retry()`
    );
    const logs: unknown[] = [];
    try {
      const failure = await runtime
        .runPromise(
          Effect.flatMap(PictureStore, (store) => store.settleImport(job, false, now)).pipe(
            Effect.provide(
              Logger.layer([
                Logger.make(({ message }) => {
                  logs.push(message);
