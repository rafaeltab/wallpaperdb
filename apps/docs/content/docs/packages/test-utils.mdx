...
The `@wallpaperdb/test-utils` package provides the TesterBuilder pattern framework - a composable, type-safe system for building test infrastructure. It includes 10 infrastructure builders that can be mixed together to create exactly the test environment you need.
...
```typescript
import { PostgresTesterBuilder, S3TesterBuilder, NatsTesterBuilder } from "@wallpaperdb/test-utils";
...
| `S3TesterBuilder` | S3/SeaweedFS storage | S3 client, bucket management |
| `NatsTesterBuilder` | NATS messaging | NATS connection, JetStream |
| `RedisTesterBuilder` | Redis cache | Redis client |
| `OpenSearchTesterBuilder` | OpenSearch search engine | OpenSearch client, configuration |
...
S3 storage with bucket management. Tests start `chrislusf/seaweedfs:4.47` and use the AWS SDK S3 client through the `tester.s3` namespace.
...
**Methods:**
- `withS3(configure?)` - Configure a SeaweedFS container to start during `setup()`
- `withS3Bucket(name)` - Add a bucket to create; call once per bucket
- `withS3AutoCleanup()` - Remove bucket contents when `cleanup()` runs
- `getS3()` - Read the container configuration, also available as `tester.s3.config`

**Configuration callback:**
- `withImage(image)` - Choose a compatible SeaweedFS image
- `withAccessKey(key)` / `withSecretKey(key)` - Set credentials; both default to `storageadmin`
- `withNetworkAlias(alias)` - Set the Docker network alias; defaults to `s3`

**Provides:**
- `s3.getS3Client()` - Cached AWS SDK v3 S3 client
- `s3.uploadObject()`, `s3.deleteObject()`, `s3.objectExists()`, `s3.listObjects()` - Object helpers
- `s3.config.container` - Container instance
- `s3.config.endpoints` - Endpoints for host and Docker clients
...
OpenSearch search engine for full-text search and analytics.

```typescript
import { OpenSearchTesterBuilder } from "@wallpaperdb/test-utils";
import { Client } from "@opensearch-project/opensearch";

const tester = await new OpenSearchTesterBuilder()
  .withOpenSearch({
...
const client = new Client({
  node: tester.opensearch.config.endpoint.fromHost,
  auth: {
    username: tester.opensearch.config.username,
    password: tester.opensearch.config.password
  },
  ssl: {
    rejectUnauthorized: false  // For self-signed cert in tests
  }
});
...
**Methods:**
- `withOpenSearch(options?)` - Start OpenSearch container

**Options:**
- `image` - Docker image (default: `'opensearchproject/opensearch:2'`)
- `networkAlias` - Network alias (default: `'opensearch'`)

**Provides:**
- `opensearch.config` - Complete configuration object
  - `container` - Container instance
  - `endpoint` - Endpoint URLs (networked, fromHost, directIp)
...
// tester.pool, tester.s3, tester.nats, tester.redis
```
...
// TypeScript knows tester has both pool and s3
tester.pool;  // ✓ Type: Pool
tester.s3.getS3Client(); // ✓ Type: S3Client
