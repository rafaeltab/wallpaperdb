...
export function beginInvocation(result,invocation){
  if(result.finishedAt&&result.invocations?.length)result.invocations.at(-1).finishedAt??=result.finishedAt;
  delete result.finishedAt;
  result.invocations??=[];result.invocations.push(invocation);
}
