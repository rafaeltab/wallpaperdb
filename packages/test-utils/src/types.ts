...
export type TupleToIntersection<T extends readonly unknown[]> =
    ...
    : unknown;
export type TupleToIntersectionOfConstructors<T extends readonly unknown[]> = T extends [infer Head, ...infer Tail] ? AnyConstructorFor<Head> & TupleToIntersectionOfConstructors<Tail> : {};
...
type Includes<T extends readonly unknown[], V> =
    V extends TupleToUnion<T> ? true : false;

// Helper: push unique values into an accumulator (set semantics)
type PushIfNotIncluded<
    Acc extends readonly unknown[],
    V
> = Includes<Acc, V> extends true ? Acc : [...Acc, V];
...
export type JoinStrings<T extends readonly string[], Sep extends string = ", "> =
    T extends [] ? "" :
