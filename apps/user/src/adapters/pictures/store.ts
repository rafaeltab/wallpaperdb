...
export const pictureStoreLayer = (config: { readonly bucket: string }) =>
  Layer.effect(
    PictureStore,
    Effect.gen(function* () {
      ...
      const query = <A>(
        ...
        Effect.tryPromise({
          ...
          catch: (cause) => new PictureUnavailable({ operation, cause: databaseDiagnostic(cause) }),
        }).pipe(
        ...
        finishDeletion: (id) =>
          query('finish-deletion', async (db) => {
            const rows = await db
              ...
              .where(
                and(eq(profilePictureAssets.id, id), eq(profilePictureAssets.state, 'deleting'))
              )
              .returning({ id: profilePictureAssets.id });
            return rows.length === 1;
          }),
