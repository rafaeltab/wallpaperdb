...
class OpenSearchHelpers {
  ...
  get config(): OpenSearchConfig {
    ...
    if (!config) {
      throw new Error('OpenSearch not initialized. Call withOpenSearch() and setup() first.');
    }
    return config;
  }
    ...
    return class OpenSearch extends Base {
      ...
      withOpenSearch(configure: (opensearch: OpenSearchBuilder) => OpenSearchBuilder = (a) => a) {
        ...
        this.addSetupHook(async () => {
          ...
          let container = new OpenSearchContainer(image);
          ...
          const endpoint = {
            networked: `https://${host.networked}:${port.networked}`,
            fromHost: `https://${host.fromHost}:${port.fromHost}`,
            fromHostDockerInternal: `https://${host.fromHostDockerInternal}:${port.fromHostDockerInternal}`,
            directIp: `https://${host.directIp}:${port.directIp}`,
          };
