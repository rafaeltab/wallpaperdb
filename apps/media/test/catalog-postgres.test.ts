...
describe('PostgreSQL catalog contract', () => {
  ...
  it('projects late asset metadata without rolling back a newer profile snapshot', async () => {
    await runtime.runPromise(Effect.gen(function* () {
      const projection = yield* CatalogProjection;
      const catalog = yield* Catalog;
      const current: ProjectionInput = { kind: 'profile', occurrence: { source: 'users', id: 'profile-v3' }, occurredAt: '2026-01-03T00:00:00.000Z', profile: { id: 'profile-one', version: 3, pictureId: 'picture-new', updatedAt: '2026-01-03T00:00:00.000Z' } };
      yield* projection.accept(current);
      expect(yield* catalog.findCurrentPicture('picture-new')).toBeNull();
      yield* projection.accept({ ...current, occurrence: { source: 'users', id: 'profile-v2' }, profile: { ...current.profile, version: 2 }, asset: { id: 'picture-new', storageBucket: 'pictures', storageKey: 'new', mimeType: 'image/webp', width: 256, height: 256, fileSizeBytes: 100, createdAt: '2026-01-02T00:00:00.000Z' } });
      expect(yield* catalog.findCurrentPicture('picture-new')).toMatchObject({ storageKey: 'new' });
      yield* projection.accept({ ...current, occurrence: { source: 'users', id: 'profile-v1' }, profile: { ...current.profile, version: 1, pictureId: 'picture-old' }, asset: { id: 'picture-old', storageBucket: 'pictures', storageKey: 'old', mimeType: 'image/webp', width: 256, height: 256, fileSizeBytes: 100, createdAt: '2026-01-01T00:00:00.000Z' } });
      expect(yield* catalog.findCurrentPicture('picture-old')).toBeNull();
      expect(yield* catalog.findCurrentPicture('picture-new')).toMatchObject({ storageKey: 'new' });
      yield* projection.accept({ ...current, occurrence: { source: 'users', id: 'profile-v4' }, profile: { ...current.profile, version: 4, pictureId: null } });
      expect(yield* catalog.findCurrentPicture('picture-new')).toBeNull();
      yield* projection.accept(current);
      expect(yield* catalog.findCurrentPicture('picture-new')).toBeNull();
    }));
  ...
  it('rolls back the processed occurrence when persistence fails so the input can be retried', async () => {
    if (wallpaper.kind !== 'wallpaper') throw new Error('invalid fixture');
    const invalid: ProjectionInput = { ...wallpaper, wallpaper: { ...wallpaper.wallpaper, width: 2147483648 } };
  ...
  it('selects the smallest area meeting both dimensions without integer overflow', async () => {
    await runtime.runPromise(Effect.gen(function* () {
      ...
      for (const choice of choices) yield* projection.accept({ kind: 'variant', occurrence: { source: 'generator', id: choice.storageKey }, occurredAt: '2026-01-01T00:00:01.000Z', variant: { wallpaperId: 'wlpr_one', storageBucket: 'wallpapers', mimeType: 'image/jpeg', fileSizeBytes: 100, createdAt: '2026-01-01T00:00:01.000Z', ...choice } });
      expect(yield* catalog.findSmallestVariant('wlpr_one', 50000, 50000)).toMatchObject({ storageKey: 'small' });
      expect(yield* catalog.findSmallestVariant('wlpr_one', 60000, 50000)).toMatchObject({ storageKey: 'large' });
