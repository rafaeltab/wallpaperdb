...
afterEach(async () => {
  ...
  await manager.streams.purge('VARIANT_GENERATOR_QUARANTINE');
  await manager.consumers.delete('WALLPAPER', 'variant-generator-wallpaper-uploaded-consumer');
});
      ...
      Layer.provide(
        Layer.succeed(GenerateVariants, {
          generate: (input) =>
            Effect.sync(() => {
              calls.push(input);
              return input.fileType === 'video'
                ? { _tag: 'Skipped', reason: 'not_image' }
                : { _tag: 'Generated', variants: [] };
            }),
        })
      )
    )
...
it('quarantines invalid input and exhausted failures after exactly three delayed attempts', async () => {
  ...
  try {
    ...
    const original = await manager.streams.getMessage('VARIANT_GENERATOR_QUARANTINE', {
...
it('keeps exhausted input pending until quarantine accepts it without processing a fourth time', async () => {
  ...
  try {
    ...
    await js.publish('variant-generator.quarantine', 'full');
...
it('quarantines malformed 64 KiB payloads byte for byte without invoking generation', async () => {
  ...
  try {
    ...
    await expect
      .poll(async () => (await manager.streams.info('VARIANT_GENERATOR_QUARANTINE')).state.messages)
      .toBe(1);
    const message = await manager.streams.getMessage('VARIANT_GENERATOR_QUARANTINE', {
      last_by_subj: 'variant-generator.quarantine',
    });
...
it('translates canonical CloudEvents and retries technical failures without acknowledging incomplete work', async () => {
  const calls: GenerationInput[] = [];
  const options = {
    url: tester.nats.config.endpoints.fromHost,
    stream: 'WALLPAPER',
  ...
  const runtime = ManagedRuntime.make(
    natsConsumerLayer(options).pipe(
      ...
      Layer.provide(Layer.succeed(GenerateVariants, {
        generate: (input) => Effect.suspend(() => {
          calls.push(input);
          return calls.length === 1
            ? Effect.fail(new GenerationUnavailable({ operation: 'publish-variant', cause: 'unavailable' }))
            : Effect.succeed({ _tag: 'Generated', variants: [] });
        }),
      })),
    ),
  );
  try {
    await runtime.runPromise(ConsumerHealth);
    const event = {
      specversion: '1.0', source: 'https://wallpaperdb/ingestor', id: 'canonical',
      type: 'wallpaper.uploaded', time: '2026-09-24T10:00:00.000Z',
      datacontenttype: 'application/json', correlationid: 'workflow', causationid: 'command',
      data: { wallpaper: upload('canonical').wallpaper },
    };
    await (await tester.nats.getJsClient()).publish('wallpaper.uploaded', JSON.stringify(event));
    await expect.poll(() => calls.length).toBe(2);
    expect(calls[0]).toMatchObject({
      occurrence: { source: event.source, id: event.id },
      correlationId: 'workflow', causationId: 'command',
      width: 2, height: 2, mimeType: 'image/png',
    });
