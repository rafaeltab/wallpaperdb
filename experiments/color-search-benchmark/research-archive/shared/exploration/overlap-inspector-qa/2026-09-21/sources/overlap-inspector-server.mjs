...
export function createOverlapInspectorServer({ corpus, methods, search, inspect, presets = overlapPresets, namedColors = NAMED_COLORS, requestTimeoutMs = 30000 }) {
  ...
  return createServer(async (request, response) => {
    ...
    try {
      ...
      if (['/api/search', '/api/inspect'].includes(url.pathname)) {
        ...
        try {
          result = inspecting
            ? await inspect({ method, id: body.id, query, signal: controller.signal })
            : await search({ method, query, limit, includeFixtures: body.includeFixtures ?? false, signal: controller.signal });
        ...
        if (inspecting) {
          ...
          send(response, 200, { ...result, hits, query, methodId: method, elapsedMs, exceedsOneSecond: elapsedMs >= 1000 });
        }
        return;
      }
      if (!['GET', 'HEAD'].includes(request.method)) throw failure('Use a read request.', 405);
      if (url.pathname === '/api/config') {
        send(response, 200, { methods, corpus: { total: corpus.length, real: corpus.length - fixtures, fixtures }, presets, namedColors, comparisonUrl: 'http://zerotwo:8225/' });
        return;
      }
