...
export function BiographyMarkdown({
  ...
  return (
    <BiographyContext.Provider value={{ profileId, refreshKey }}>
...
const BiographyContext = createContext({ profileId: '', refreshKey: 0 });

function BiographySpan({ node, children }: ComponentProps<'span'> & ExtraProps) {
  const { profileId, refreshKey } = useContext(BiographyContext);
  const wallpaperId = node?.properties.dataWallpaperId;
  return typeof wallpaperId === 'string' ? (
    <BiographyWallpaper
      key={`${profileId}:${wallpaperId}:${refreshKey}`}
...
const biographyComponents: Components = {
  ...
  pre: ({ children }) => (
    <pre className="max-w-full overflow-x-auto rounded-lg bg-muted p-4 text-sm">{children}</pre>
  ),
  code: ({ children }) => (
