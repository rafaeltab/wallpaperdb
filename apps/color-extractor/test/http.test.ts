...
const services=availabilityLayer.pipe(Layer.provide(Layer.succeed(AvailabilityProbe,{
...
it('serves health, readiness, OpenAPI, CORS and safe errors',async()=>{
  ...
  try {
    expect((await app.inject('/health')).json()).toMatchObject({status:'healthy',checks:{s3:true,nats:true,otel:true,consumer:true}});
    ...
    expect((await app.inject({url:'/health',headers:{origin:'http://localhost:3000'}})).headers['access-control-allow-origin']).toBe('http://localhost:3000');
    expect((await app.inject({url:'/health',headers:{origin:'http://evil-localhost:3000'}})).headers['access-control-allow-origin']).toBeUndefined();
    expect((await app.inject('/missing')).json()).toMatchObject({status:404,type:expect.stringContaining('/not-found.md')});
    ...
    expect((await app.inject('/ready')).json()).toMatchObject({status:503,ready:false,reason:'Service is shutting down'});
    app.connectionsState.isShuttingDown=false;
    app.connectionsState.connectionsInitialized=false;
    expect((await app.inject('/ready')).json()).toMatchObject({ready:false,reason:'Service is not yet initialized'});
  } finally {await app.close();}
});
it('hides defects and releases scoped services on close',async()=>{
  let closed=false;
  const layer=Layer.effect(Availability,Effect.gen(function*(){
    yield* Effect.addFinalizer(()=>Effect.sync(()=>{closed=true;}));
    return Availability.of({health:()=>Effect.die(new Error('secret')),ready:()=>Effect.die(new Error('secret'))});
  }));
