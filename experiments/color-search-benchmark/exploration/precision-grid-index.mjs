...
export const PRECISION_GRID_INDEX='color-exploration-precision-grid-real-v2';
...
export async function precisionGridFingerprints(){
  return {descriptorHash:hash(await readFile(path.join(CORPUS_STORE,'features.jsonl'))),definitionHash:hash(PRECISION_GRID_DEFINITION),computationHash:hash({definition:PRECISION_GRID_DEFINITION,encoder:createPrecisionGridEncoder.toString(),colorTransform:rgbToLab.toString(),utilityKey:precisionGridKey.toString()}),sourceHashes:Object.fromEntries(await Promise.all(PRECISION_GRID_SOURCE_FILES.map(async name=>[name,hash(await readFile(new URL(name,import.meta.url)))])))};
}
export function validatePrecisionGridMetadata(metadata,fingerprints){
  for(const field of ['descriptorHash','definitionHash','computationHash'])if(!fingerprints[field]||metadata?.[field]!==fingerprints[field])throw Error('Precision-grid '+field+' fingerprint mismatch');
  return true;
}
export async function validatePrecisionGridIndex({index=PRECISION_GRID_INDEX,expectedIds}={}){
  ...
  validatePrecisionGridMetadata(mapping?._meta,fingerprints);
