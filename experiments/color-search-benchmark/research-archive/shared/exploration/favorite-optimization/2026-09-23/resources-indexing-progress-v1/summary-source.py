...
def summarize_benchmark_profile(profile, observations, campaign_kind):
    ...
    if not before or not after: return {**result, 'available': False, 'reason': 'Missing resource bracket'}
    samples = [before, *profile.get('samples', []), after]
    ...
    def node_sum(sample, *keys):
        ...
        for node in sample['nodes'].values():
            ...
            values.append(value)
        return sum(values)
    cpu = delta([node_sum(sample, 'process', 'cpu', 'total_in_millis') for sample in samples])
    if not stable_nodes: cpu.update(delta=None, valid=False, reason='Node identity changed')
    ...
    node = {'identityStable': stable_nodes, 'resourceBracketSeconds': seconds, 'processCpuMilliseconds': cpu,
      ...
      'searchRejections': delta([node_sum(sample, 'thread_pool', 'search', 'rejected') for sample in samples]),
    ...
    node['gc'] = {collector: {key: delta([node_sum(sample, 'jvm', 'gc', 'collectors', collector, key) for sample in samples]) for key in ['collection_count', 'collection_time_in_millis']}
    ...
    result.update(available=True, node=node, loadGenerator={'cpuMicroseconds': client_cpu,
      ...
      resourceSampleErrors=profile.get('resourceErrors', 0),
...
def main():
    ...
    for filename in args.campaign:
        ...
        finished = campaign.get('finishedAt')
        complete = bool(finished) and not campaign.get('error') and not campaign.get('interruption')
        phase = summarize_phase(observations, {'id': filename.parent.name, 'kind': campaign['experiment'],
          'startedAt': campaign['startedAt'], 'endedAt': finished or captured_at, 'complete': complete})
        profiles = [summarize_benchmark_profile(profile, observations, campaign['experiment']) for profile in campaign.get('profiles', [])]
