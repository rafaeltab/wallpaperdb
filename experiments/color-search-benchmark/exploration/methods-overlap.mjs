// THROWAWAY PROTOTYPE. OpenSearch functions order every eligible document.
...
const LIMITATIONS = Object.freeze([
  ...
  'OpenSearch scores globally without application reranking. Linear quality uses native functions; power quality may use a Painless score function. Query cost must be measured.',
]);
...
export function overlapParameters(input, supplied = {}) {
  ...
  const known = new Set(['areaPower', 'qualityPenalty', 'excessPenalty', 'qualityInfluence', 'qualityCurve', 'minimumQuality', 'bucketCount']);
  for (const name of Object.keys(supplied)) if (!known.has(name)) throw Error(`Unsupported overlapping-region parameter: ${name}`);
  const parameters = { areaPower: .5, qualityPenalty: .35, excessPenalty: 1.5, qualityInfluence: 1, qualityCurve: 'linear', minimumQuality: 0, bucketCount: 1024, ...supplied };
  ...
  if (!['linear', 'power'].includes(parameters.qualityCurve)) throw Error('qualityCurve must be linear or power.');
