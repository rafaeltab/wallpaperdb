...
const profiles: Profiles = {
  ...
  changeHandle: unused,
  reactivateAlias: unused,
  scheduleAliasExpiry: unused,
  expireAliasImmediately: unused,
  updateDetails: unused,
  adoptPicture: unused,
  adoptImportedPicture: unused,
...
it('records publication only after broker acceptance and retries the original durable event', async () => {
  ...
  const runtime = ManagedRuntime.make(
    maintenanceLayer({ retentionDays: 30 }).pipe(
      Layer.provide(
        Layer.mergeAll(
          Layer.succeed(MaintenanceStore, {
            ...
            pendingEvents: () =>
              Effect.succeed([...pending].map((id) => ({ id, createdAt: new Date(0) }))),
            markPublished: (id) =>
              Effect.sync(() => {
                pending.delete(id);
              }),
  ...
  try {
    expect(
      await runtime.runPromise(Effect.flatMap(Maintenance, (service) => service.publishPending()))
...
it('advances evidence cleanup beyond failures, wraps for retry, and uses the configured rolling cutoff', async () => {
  const pending = new Set(
    Array.from({ length: 102 }, (_, index) => `event-${String(index).padStart(3, '0')}`)
  );
          ...
          Layer.succeed(MaintenanceStore, {
            ...
            expiredEvents: (cutoff, after) =>
              Effect.sync(() => {
                ...
                return [...pending]
                  .filter((id) => !after || id > after.id)
                  .slice(0, 100)
                  .map((id) => ({ id, createdAt: new Date(0) }));
              }),
            deleteExpiredEvent: (id) =>
              Effect.suspend(() =>
                id === 'event-000' && failing
                  ? Effect.fail(
                      new MaintenanceFailure({ operation: 'delete', cause: 'unavailable' })
                    )
  ...
  try {
    const cleanup = Effect.flatMap(Maintenance, (service) =>
      service.cleanupEvents(new Date('2030-01-31T00:00:00.000Z'))
    );
      ...
      Layer.provide(
        Layer.mergeAll(
          Layer.succeed(MaintenanceStore, {
            ...
            dueAliases: () =>
              Effect.succeed(
                ['broken', 'completed', 'skipped'].map((handle) => ({
                  handle,
                  profileId: handle,
                  claimGeneration: 7,
          ...
          Layer.succeed(Profiles, {
            ...profiles,
            expireDueAlias: (claim) =>
              Effect.suspend(() => {
                attempted.push(claim.handle);
                if (claim.handle === 'broken')
                  return Effect.fail(
                    new ProfileUnavailable({ operation: 'expire', cause: 'offline' })
                  );
                stopping = true;
  ...
  try {
    expect(
      await runtime.runPromise(
        Effect.flatMap(Maintenance, (service) => service.expireAliases(new Date(), () => stopping))
      )
    ).toEqual({ completed: 1, failed: 1 });
...
it('publishes beyond a full poisoned batch and retries ambiguous completion with the same occurrence', async () => {
  const pending = new Set(
    Array.from({ length: 101 }, (_, index) => `event-${String(index).padStart(3, '0')}`)
  );
        ...
        Layer.mergeAll(
          ...
          Layer.succeed(ProfileEvents, {
            publish: (id) =>
              id < 'event-100'
                ? Effect.fail(
                    new MaintenanceFailure({ operation: 'publish', cause: 'invalid event' })
                  )
                : Effect.sync(() => {
                    accepted.add(id);
                  }),
