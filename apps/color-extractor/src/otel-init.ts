...
const shutdownSdk = (sdk: NodeSDK) =>
  Effect.tryPromise(() => sdk.shutdown()).pipe(
    ...
    Effect.catch(() => Effect.logWarning('Color extractor telemetry shutdown failed'))
  );

/** Initialize before loading adapters so auto-instrumentation sees their first imports. */
export const initializeOtel = Effect.fn('color-extractor.telemetry.initialize')(function* (config: {
  ...
  return yield* initialize.pipe(
    Effect.catchTag('OtelInitializationError', (error) =>
      Effect.logWarning('Color extractor telemetry initialization failed', {
        cause: error.cause,
