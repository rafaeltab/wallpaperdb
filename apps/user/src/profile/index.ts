...
export function reject(reason: RejectionReason, message: string): ProfileRejection {
  return { _tag: 'Rejected', reason, message };
}
export function versionConflict() {
  return reject('version-conflict', 'Profile has changed since it was last loaded');
}
...
export const profilesLayer = (policy: ProfilePolicy) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const adopt = Effect.fn('profiles.adopt-picture')(function* (
        principal: ProfilePrincipal,
        assetId: string | null,
        authorization:
          | { type: 'owner'; expectedVersion: number }
          | { type: 'import'; leaseToken: string }
      ) {
        if (!principal.profileId) return reject('unauthorized', 'Authentication is required');
        const now = new Date(yield* Clock.currentTimeMillis);
        const result = yield* store.transact(
          { profileId: principal.profileId, assetId, now },
          ({ profile, importJob, asset, observedAt: now }) => {
            if (authorization.type === 'import') {
              if (
                !authorization.leaseToken ||
                !importJob ||
                importJob.leaseToken !== authorization.leaseToken ||
                importJob.status === 'complete' ||
                profile.pictureAssetId !== null
              )
                return { _tag: 'Unchanged' };
            } else if (profile.version !== authorization.expectedVersion) return versionConflict();
            if (
              profile.pictureAssetId === assetId &&
            ...
            if (
              assetId &&
              (!asset ||
                asset.profileId !== principal.profileId ||
                asset.state !== 'staged' ||
                !asset.expiresAt ||
                asset.expiresAt <= now)
            )
              return reject('picture-unavailable', 'The staged Profile picture is unavailable');
            return {
              _tag: 'Change',
              mutation: {
                type: 'picture',
                asset,
                source:
                  authorization.type === 'import' ? 'clerk-import' : assetId ? 'upload' : 'remove',
              },
            };
          }
        );
      ...
      const mutateAlias = Effect.fn('profiles.mutate-alias')(function* (
        principal: ProfilePrincipal,
        requestedHandle: string,
        expectedVersion: number,
        operation: 'reactivate' | 'schedule' | 'expire'
      ) {
        if (!principal.profileId) return reject('unauthorized', 'Authentication is required');
        if (!Number.isInteger(expectedVersion) || expectedVersion < 1)
          return reject(
            'invalid-alias-command',
            'Expected Profile version must be a positive integer'
          );
        const handle = requestedHandle.toLowerCase();
        const now = new Date(yield* Clock.currentTimeMillis);
        const result = yield* store.transact(
          { profileId: principal.profileId, now, handle },
          (snapshot) => {
      ...
      const changeHandle = Effect.fn('profiles.change-handle')(function* (
        principal: ProfilePrincipal,
        requestedHandle: string,
        expectedVersion: number
      ) {
        if (!principal.profileId) return reject('unauthorized', 'Authentication is required');
        if (!Number.isInteger(expectedVersion) || expectedVersion < 1)
          return reject('invalid-handle', 'Expected Profile version must be a positive integer');
        const handle = slugify(requestedHandle);
        if (
          handle.length < policy.profileHandleMinLength ||
          handle.length > policy.profileHandleMaxLength
        )
          return reject(
            'invalid-handle',
            `Handle must contain ${policy.profileHandleMinLength}–${policy.profileHandleMaxLength} letters, numbers, or single hyphens after normalization`
          );
        if (reserved.has(handle))
          return reject('invalid-handle', 'This Handle is reserved; choose another name');
        const now = new Date(yield* Clock.currentTimeMillis);
        const result = yield* store.transact(
          { profileId: principal.profileId, now, handle },
          ({ profile, targetClaim, observedAt: now }) => {
            ...
            if (profile.lastHandleChangedAt) {
              const nextHandleChangeAt = new Date(
                profile.lastHandleChangedAt.getTime() + 7 * 86400000
              );
              if (now < nextHandleChangeAt)
                return {
                  ...reject('handle-cooldown', 'You can change your Handle once every seven days'),
                  nextHandleChangeAt,
                };
            }
            if (targetClaim && targetClaim.profileId !== principal.profileId)
              return reject(
                'handle-unavailable',
                'This Handle is already in use; choose another name'
              );
            const retained = profile.aliases.filter(
              (alias) => alias.expiresAt === null && alias.handle !== handle
            );
            retained.push({
            ...
            const scheduledAliases = retained
              .slice(0, Math.max(0, retained.length - policy.profileRetainedAliasLimit))
              .map((alias) => ({
                handle: alias.handle,
      ...
      const updateDetails = Effect.fn('profiles.update-details')(function* (
        principal: ProfilePrincipal,
        changes: { displayName?: string; biographyMarkdown?: string },
        expectedVersion: number
      ) {
        if (!principal.profileId) return reject('unauthorized', 'Authentication is required');
        const displayName =
          changes.displayName === undefined ? undefined : normalizeName(changes.displayName);
        if (displayName !== undefined) {
          if (!displayName) return reject('invalid-display-name', 'Display name must not be blank');
          if ([...displayName].length > policy.profileDisplayNameMaxLength)
            return reject(
              'invalid-display-name',
              `Display name must be at most ${policy.profileDisplayNameMaxLength} characters`
            );
        }
        ...
        if (biographyMarkdown !== undefined) {
          const validation = validateProfileMarkdown(biographyMarkdown, {
            maxCharacters: policy.profileBiographyMaxLength,
          });
          if (!validation.valid)
            return reject(
              'invalid-biography',
        ...
        const now = new Date(yield* Clock.currentTimeMillis);
        const result = yield* store.transact(
          { profileId: principal.profileId, now, wallpaperIds },
          ({ profile, wallpaperOwners }) => {
            if (profile.version !== expectedVersion) return versionConflict();
            if (
              (displayName === undefined || profile.displayName === displayName) &&
              (biographyMarkdown === undefined || profile.biographyMarkdown === biographyMarkdown)
            )
              return { _tag: 'Unchanged' };
            if (biographyMarkdown !== profile.biographyMarkdown && wallpaperIds.length > 0) {
              if (wallpaperOwners.some((wallpaper) => wallpaper.profileId !== principal.profileId))
                return {
                  ...reject(
                    'unavailable-biography-wallpaper',
              ...
              if (wallpaperOwners.length !== wallpaperIds.length)
                return {
                  ...reject(
                    'unavailable-biography-wallpaper',
            ...
            return {
              ...
              mutation: {
                ...
                biographyMarkdown: biographyMarkdown ?? profile.biographyMarkdown,
              },
            };
          }
        );
      ...
      return Profiles.of({
        updateDetails,
        adoptPicture: (principal, assetId, expectedVersion) =>
          adopt(principal, assetId, { type: 'owner', expectedVersion }),
        adoptImportedPicture: (lease, assetId) =>
          adopt(lease, assetId, { type: 'import', leaseToken: lease.leaseToken }),
        changeHandle,
        reactivateAlias: (principal, handle, version) =>
          mutateAlias(principal, handle, version, 'reactivate'),
        scheduleAliasExpiry: (principal, handle, version) =>
          mutateAlias(principal, handle, version, 'schedule'),
        expireAliasImmediately: (principal, handle, version) =>
          mutateAlias(principal, handle, version, 'expire'),
        expireDueAlias: (reference, now) =>
          store
            .transact(
              { profileId: reference.profileId, now, handle: reference.handle },
              ({ profile, targetClaim }) => {
                if (
                  !targetClaim ||
                  targetClaim.profileId !== profile.id ||
                  targetClaim.kind !== 'alias' ||
                  targetClaim.claimGeneration !== reference.claimGeneration ||
                  !targetClaim.expiresAt ||
                  targetClaim.expiresAt > now
                )
                  return { _tag: 'Unchanged' };
                return {
                  _tag: 'Change',
                  mutation: {
                    ...
                    claimGeneration: reference.claimGeneration,
                    before: targetClaim.expiresAt.toISOString(),
                    reason: 'scheduled',
                  },
                };
              }
          ...
          const displayName = [
            ...(normalizeName(identity.displayName ?? '') || fullName || fallback),
          ]
            .slice(0, policy.profileDisplayNameMaxLength)
            .join('')
            .trim();
          const slug = (slugify(displayName) || `profile-${seed}`).padEnd(
          ...
          for (let attempt = 0; attempt < 100; attempt++) {
            const handle =
              attempt === 0
                ? base
                    .slice(0, policy.profileHandleMaxLength)
                    .replace(/-+$/g, '')
            ...
            const profile = yield* store.create({
          ...
          return yield* Effect.fail(
            new ProfileUnavailable({
              operation: 'claim-initial-handle',
              cause: new Error('Handle attempts exhausted'),
            })
          );
        }),
      });
...
function decideAlias(
  { profile, targetClaim, eligibleHandles }: ProfileSnapshot,
  handle: string,
  operation: 'reactivate' | 'schedule' | 'expire',
  now: Date,
  ...
  if (operation === 'reactivate') {
    if (profile.handle === handle)
      return reject('invalid-alias-command', 'Your current Handle cannot also be an alias');
    if (!eligibleHandles.includes(handle))
      return reject('ineligible-handle', 'This Handle is outside your retained Profile history');
    if (targetClaim && (targetClaim.profileId !== profile.id || targetClaim.kind !== 'alias'))
      return reject('handle-unavailable', 'This Handle is already claimed');
    if (targetClaim && !targetClaim.expiresAt) return { _tag: 'Unchanged' };
    if (
      profile.aliases.filter((alias) => alias.expiresAt === null).length >=
      policy.profileRetainedAliasLimit
    )
      return reject('alias-limit', 'No retained alias slot is available');
    return {
      _tag: 'Change',
      mutation: {
        type: 'reactivate',
        handle,
        before: targetClaim?.expiresAt?.toISOString() ?? null,
      },
    };
  }
  if (!targetClaim || targetClaim.profileId !== profile.id || targetClaim.kind !== 'alias')
    return reject('alias-not-found', 'This Handle is not one of your aliases');
  if (operation === 'schedule') {
    if (targetClaim.expiresAt) return { _tag: 'Unchanged' };
    return {
      _tag: 'Change',
      mutation: { type: 'schedule', handle, expiresAt: new Date(now.getTime() + 86400000) },
    };
  }
  if (!targetClaim.expiresAt)
    return reject(
      'alias-not-scheduled',
  ...
  return {
    ...
    mutation: {
      type: 'expire',
      handle,
      claimGeneration: targetClaim.claimGeneration,
      before: targetClaim.expiresAt.toISOString(),
      reason: 'immediate',
    },
  };
...
export function describeOwnerProfile(
  ...
  const retained = new Set(
    aliases.filter((alias) => alias.expiresAt === null).map((alias) => alias.handle)
  );
  ...
  return {
    ...
    historicalHandles: history
      .filter((entry) => entry.handle !== profile.handle && !retained.has(entry.handle))
      .map((entry) => ({
        ...entry,
        unavailableReason: claimedByOthers.has(entry.handle)
          ? 'claimed'
          : retained.size >= policy.profileRetainedAliasLimit
            ? 'alias-limit'
            : null,
      })),
  };
