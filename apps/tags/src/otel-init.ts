...
const shutdownSdk = (sdk: NodeSDK) =>
  Effect.tryPromise(() => sdk.shutdown()).pipe(
    ...
    Effect.catch(() => Effect.logWarning('Tags telemetry shutdown failed'))
  );
...
export const initializeOtel = Effect.fn('tags.telemetry.initialize')(function* (config: {
  ...
  const initialize = Effect.gen(function* () {
    ...
    yield* Effect.acquireRelease(acquireSdk, shutdownSdk);
    return { _tag: 'Started' } as const;
  });
  return yield* initialize.pipe(
    Effect.catchTag('OtelInitializationError', () =>
      Effect.logWarning('Tags telemetry initialization failed').pipe(
