...
const owner: OwnerProfile = {
  ...
  pictureUploadLimits: { maxBytes: 5242880, maxPixels: 16000000, maxDecodedBytes: 67108864 },
};

function controlled(
  customPolicy: ProfilePolicy = policy,
  ...
  const store: Store = {
    ...
    transact: (_query, decide) =>
      Effect.sync(() => {
        const decision = decide({
          observedAt: _query.now,
          profile: current,
          targetClaim: null,
          eligibleHandles: [],
          wallpaperOwners: [],
        ...
        if (decision._tag === 'Change') {
          if (decision.mutation.type !== 'details')
            throw new Error('Unsupported controlled transition');
          mutations.push(decision.mutation);
          current = {
            ...current,
            displayName: decision.mutation.displayName,
            biographyMarkdown: decision.mutation.biographyMarkdown,
            version: current.version + 1,
            updatedAt: _query.now,
          };
        }
        return {
          outcome: { _tag: 'Success', profile: current },
          changed: decision._tag === 'Change',
        };
      }),
  };
  const layer = profilesLayer(customPolicy).pipe(
    Layer.provide(
      Layer.mergeAll(
  ...
  return {
    created,
    mutations,
    run: <A, E>(use: (profiles: Profiles) => Effect.Effect<A, E>) =>
      Effect.runPromise(
        Effect.gen(function* () {
...
describe('Profiles capability', () => {
  it('keeps a generated handle inside the configured minimum when truncation removes a trailing hyphen', async () => {
    const test = controlled(
      { ...policy, profileHandleMinLength: 3, profileHandleMaxLength: 3 },
      { displayName: 'Ab cd', firstName: null, lastName: null }
    );
  ...
  it('normalizes changed details once and preserves the version on a repeated no-op', async () => {
    ...
    expect(test.mutations).toEqual([
      { type: 'details', displayName: 'Ada Lovelace', biographyMarkdown: '' },
    ]);
  });
