...
export async function renderFindings({corpusStore=CORPUS_STORE,explorationStore=STORE,reportBaseUrl='http://zerotwo:8224'}={}){
  ...
  const scaleRuns=[];
  for(const root of ['scale','rank-feature-scale','clickhouse-scale','precision-grid-scale'])for(const directory of await directories(path.join(explorationStore,root))){
  ...
  const rows=METHODS.map(method=>{
    ...
    return {id:method.id,label:method.label,family:method.family,objectiveApproximation:method.objectiveApproximation??false,engine:method.engine??'opensearch',representation:method.representation,retrieval:record?.execution?.retrieval??(method.approximate?'approximate':'exact-stored-objective'),record:record??null,comparison:record?sameCases(record,baseline):{available:false,reason:'No valid default run saved.'},scaleProfiles:profiles,million:profiles.filter(profile=>profile.count===1000000),largestTestedCount:profiles.length?Math.max(...profiles.map(profile=>profile.count)):null};
  });
