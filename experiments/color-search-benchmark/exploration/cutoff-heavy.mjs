...
export function cutoffHeavyVariants(config) {
  // Preserve this historical campaign's four profiles and retained projection.
  return CUTOFF_METHODS.filter(method => ['hard', 'feather', 'core-halo', 'consensus'].includes(method.profile)).flatMap(method => config.bucketCounts.map(bucketCount => ({ id: `${method.profile}-b${bucketCount}-q50`, method: method.id, parameters: { bucketCount, pixelCutoff: .5 } })));
}
