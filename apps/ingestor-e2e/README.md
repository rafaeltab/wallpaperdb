...
- Verifies the full upload flow against real infrastructure: the Docker-containerised Ingestor, PostgreSQL, SeaweedFS, and NATS must all respond correctly before a test passes
...
- **`@wallpaperdb/test-utils` TesterBuilder** — composes infrastructure builders (PostgreSQL, SeaweedFS, NATS, Redis, Docker) and the `ContainerizedIngestorTesterBuilder` into a single declarative test setup
