.PHONY: infra-start infra-stop infra-reset infra-logs apps-start apps-stop \
        ...
        dev build test test-watch test-unit test-integration test-e2e test-ui nats-container-test storage-test storage-infra-test coverage-summary format lint check-types ci ci-force clean help
...
help:
	...
	@echo "  make nats-container-test - Test NATS container readiness (TEST_ARGS for Vitest options)"
...
nats-container-test:
	@$(TURBO) run build --filter='@wallpaperdb/testcontainers^...'
	@pnpm --filter @wallpaperdb/testcontainers test test/nats-container.test.ts $(TEST_ARGS)
