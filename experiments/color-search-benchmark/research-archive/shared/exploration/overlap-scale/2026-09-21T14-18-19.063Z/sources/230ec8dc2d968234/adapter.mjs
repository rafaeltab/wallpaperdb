  ...
  return {
    ...
    async prepare() {
      if (method.searchKind === 'overlap') {
        const verified = await verifyOverlap({ index, expectedIds: context.corpus.map(asset => asset.id) });
