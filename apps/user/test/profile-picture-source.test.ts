...
async function downloadInitialPicture(
  ...
  const result = await Effect.runPromise(
    Effect.flatMap(PictureSource, (source) => source.download(url)).pipe(
...
const options = {
  maxBytes: 1024,
  timeoutMs: 1000,
  allowedHosts: ['img.clerk.com', 'images.clerk.dev'],
};
...
describe('Initial Profile picture download', () => {
  it('downloads the captured source through the configured external fetch boundary', async () => {
    ...
    expect(
      await downloadInitialPicture('https://img.clerk.com/captured-picture', options, fetcher)
    ).toEqual(bytes);
    expect(fetcher).toHaveBeenCalledWith(
      'https://img.clerk.com/captured-picture',
  ...
  it('rejects untrusted initial destinations before sending any request', async () => {
    ...
    for (const url of [
      'not a url',
      'http://img.clerk.com/picture',
      'https://img.clerk.com.evil.example/picture',
      'https://evil.example/img.clerk.com',
      'https://user:secret@img.clerk.com/picture',
      'https://img.clerk.com:444/picture',
      'https://127.0.0.1/picture',
      'https://[::1]/picture',
    ]) {
      await expect(
        downloadInitialPicture(
          url,
          { ...options, allowedHosts: [...options.allowedHosts, '127.0.0.1', '[::1]'] },
          fetcher
        )
  ...
  it('validates every redirect destination before following it', async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        new Response(null, { status: 302, headers: { Location: 'https://images.clerk.dev/next' } })
      )
    ...
    expect(await downloadInitialPicture('https://img.clerk.com/start', options, fetcher)).toEqual(
      Buffer.from('redirected bytes')
    );
    expect(fetcher.mock.calls.map(([url]) => String(url))).toEqual([
      'https://img.clerk.com/start',
      'https://images.clerk.dev/next',
      'https://images.clerk.dev/final',
    ]);
    for (const destination of [
      'http://img.clerk.com/picture',
      'https://127.0.0.1/private',
      'https://untrusted.example/picture',
    ]) {
      const malicious = vi
        .fn<typeof fetch>()
        .mockResolvedValue(new Response(null, { status: 302, headers: { Location: destination } }));
      await expect(
        downloadInitialPicture('https://img.clerk.com/start', options, malicious)
      ).rejects.toBeInstanceOf(PermanentPictureImportError);
    ...
    for (const hops of [3, 4]) {
      ...
      for (let index = 0; index < hops; index++) {
        fetcher.mockResolvedValueOnce(
          new Response(null, { status: 302, headers: { Location: `/hop-${index}` } })
        );
      }
  ...
  it('rejects an oversized declared body and cancels it before reading picture bytes', async () => {
    ...
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValue(new Response(body, { headers: { 'Content-Length': '1025' } }));
    await expect(
      downloadInitialPicture('https://img.clerk.com/picture', options, fetcher)
    ).rejects.toBeInstanceOf(PermanentPictureImportError);
  ...
  it('enforces actual streamed bytes with absent or understated length headers and accepts the exact limit', async () => {
    ...
    await expect(
      downloadInitialPicture('https://img.clerk.com/picture', { ...options, maxBytes: 5 }, fetcher)
  ...
  it.each([
    ...
    'body',
    'redirects',
  ])('bounds hanging %s with one shared download deadline', async (stage) => {
    vi.useFakeTimers();
    try {
      const cancel = vi.fn();
      const fetcher = vi.fn<typeof fetch>();
      if (stage === 'headers') fetcher.mockImplementation(() => new Promise(() => {}));
        ...
        fetcher.mockImplementationOnce(
          () => new Promise((resolve) => setTimeout(() => resolve(new Response('late bytes')), 60))
        );
      }
      let failure: unknown;
      void downloadInitialPicture(
        'https://img.clerk.com/picture',
  ...
  it('distinguishes permanent HTTP rejection from retryable service failures', async () => {
    for (const status of [400, 401, 403, 404, 410, 422, 408, 429, 500, 502, 503]) {
      const fetcher = vi
        .fn<typeof fetch>()
        .mockResolvedValue(new Response('not picture bytes', { status }));
      const failure = await downloadInitialPicture(
        'https://img.clerk.com/private-source',
        options,
        fetcher
      ).catch((error: unknown) => error);
      expect(failure).toBeInstanceOf(Error);
      if (status < 500 && status !== 408 && status !== 429)
        expect(failure).toBeInstanceOf(PermanentPictureImportError);
  ...
  it('keeps external failure details containing the private captured source out of retry errors', async () => {
    ...
    const failure = await downloadInitialPicture(source, options, fetcher).catch(
      (error: unknown) => error
    );
    expect(failure).toBeInstanceOf(Error);
    expect(failure).not.toBeInstanceOf(PermanentPictureImportError);
  ...
  it('propagates Effect interruption to the source request and streamed body', async () => {
    ...
    const running = Effect.runPromise(
      Effect.flatMap(PictureSource, (source) =>
        source.download('https://img.clerk.com/picture')
      ).pipe(Effect.provide(pictureSourceLayer(options, fetcher))),
