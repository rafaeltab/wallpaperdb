...
- **Public Profile reads** — projects versioned Profile events into a dedicated OpenSearch index. Exact current Handle and alias lookups return a `HandleResolution` with the Profile, requested Handle, alias indicator, and canonical Handle. Alias lists stay private; duplicate projected claims resolve by the highest generation of the matched Handle.
- **Profile discovery** — `searchProfiles(query, first, after)` searches current Handles, active aliases, and Display names. Results rank exact current Handles, current Handle prefixes, exact aliases, alias prefixes, Display name phrases/prefixes, then fuzzy Display names. Fixed relevance tiers prevent multiple weaker matches from outranking a stronger match, and Profile ID breaks ties. Biography is excluded. Search cursors bind the normalized query, score, and ID; queries ignore surrounding whitespace, case, and one optional leading `@`, then accept 1–100 characters. Pages default to 10 with a maximum of 50. Selecting a result filters wallpapers by its exact Profile ID.
- **Alias expiry** — scheduled redirects stop at their expiry timestamp even while the expiry event is still being projected. Expiry events remove alias routing, older events cannot restore it, and reclaimed Handles resolve to the newest claim generation. Retained aliases without an expiry timestamp remain active.
- **Additive index updates** — applies declared mappings to existing indexes before starting consumers, preserving projected documents when new fields such as Handle aliases are introduced
