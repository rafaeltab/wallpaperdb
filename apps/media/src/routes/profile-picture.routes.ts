...
export async function registerProfilePictureRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { pictureId: string } }>('/profile-pictures/:pictureId', async (request, reply) => {
    ...
    try {
      ...
      if (!bytes) {
        return reply.code(404).type('application/problem+json').send({
          ...
          title: 'Profile picture not found', status: 404,
          detail: 'This Profile picture is not publicly available.', instance: request.url,
        });
      }
      ...
      return reply.code(503).type('application/problem+json').send({
        ...
        title: 'Profile picture unavailable', status: 503,
        detail: 'Profile picture delivery is temporarily unavailable. Try again.', instance: request.url,
      });
    }
  });
}
