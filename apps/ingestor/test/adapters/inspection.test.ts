...
describe('image inspection', () => {
  ...
  it.effect('rejects corrupt images and formats excluded by policy', () =>
    Effect.gen(function* () {
      const inspector = yield* ContentInspection;
      const bytes = yield* Effect.promise(() => sharp({ create: {
        width: 12, height: 8, channels: 3, background: '#123456',
      } }).webp().toBuffer());
      expect(yield* inspector.inspect(bytes, 'image/webp', { ...limits, allowedFormats: [] }))
        .toEqual({ _tag: 'InvalidFormat', mimeType: 'image/webp' });
      const corrupt = Buffer.from([0xff, 0xd8, 0xff, 0xe0, 0, 16, 0x4a, 0x46, 0x49, 0x46]);
      expect(yield* inspector.inspect(corrupt, 'image/jpeg', limits))
        .toEqual({ _tag: 'InvalidFormat', mimeType: 'image/jpeg' });
    }).pipe(Effect.provide(imageInspectionLayer))
  );
  it.effect('reports dimensions outside the accepted bounds', () =>
    Effect.gen(function* () {
      ...
      const inspector = yield* ContentInspection;
      expect(yield* inspector.inspect(bytes, 'image/jpeg', { ...limits, minWidth: 20 }))
        .toEqual({ _tag: 'InvalidDimensions', width: 12, height: 8,
          minWidth: 20, minHeight: 1, maxWidth: 100, maxHeight: 100 });
    }).pipe(Effect.provide(imageInspectionLayer))
  );
  ...
  it.effect('rejects oversized undecodable content before reporting its format', () =>
    ContentInspection.use((inspector) =>
      ...
      Effect.tap((result) =>
        Effect.sync(() => expect(result).toEqual({
          _tag: 'TooLarge', fileType: 'image', fileSizeBytes: 101, maxFileSizeBytes: 100,
        }))
      ),
