...
`S3Connection` uses the AWS SDK S3 client. Configure it for SeaweedFS locally or another S3-compatible service:
...
const s3Connection = new S3Connection(config);
...
**Features:**
- Path-style URLs for SeaweedFS compatibility
- AWS SDK v3 support
- Bucket validation
...
container.register("S3Connection", {
  useFactory: () => new S3Connection(config)
});
