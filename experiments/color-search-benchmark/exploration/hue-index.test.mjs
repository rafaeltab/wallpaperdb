...
import { createHueEncoder, toHueDocument, hueMapping } from './hue-index.mjs';
...
test('encoded physical area and conditional mean agree with direct pixel membership at every anchor/layer', () => {
  ...
  const encoder = createHueEncoder({ cacheLimit: 3 });
  ...
  for (const region of OVERLAP_REGIONS) for (let level = 0; level < CUTOFF_LEVELS.length; level++) {
    ...
    for (let offset = 0; offset < rgba.length; offset += 4) {
      ...
      const match = hueMembership(pixel, region, { profile: 'hard', cutoff });
...
test('nested projections preserve literal values, metadata and named features', () => {
  const descriptor = createHueEncoder().encodePixels(Uint8Array.from([181, 55, 56, 255]));
  const feature = { id: 'fixture', cohort: 'controlled', tags: ['red'], cov_red: 10000, quality_red: .8 };
  for (const count of [16, 64, 256, 1024]) {
    const document = toHueDocument(feature, descriptor, count);
    ...
    const mapping = hueMapping({ bucketCount: count });
...
test('encoder and projection reject malformed inputs rather than emitting partial documents', () => {
  assert.throws(() => createHueEncoder({ cacheLimit: -1 }));
  assert.throws(() => createHueEncoder().encodePixels(new Uint8Array()));
  assert.throws(() => createHueEncoder().encodePixels(Uint8Array.from([1, 2, 3])));
  assert.throws(() => toHueDocument({ id: 'broken' }, { pixelCount: 1, measurements: [] }));
});
