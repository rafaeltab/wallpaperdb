...
const SHADE_LIMITATIONS = Object.freeze(LIMITATIONS.map((value, index) => index === 1
  ? 'Picked colors use the nearest indexed anchor. A separately measured shade-aware similarity relaxes lightness and partially normalizes chroma for chromatic anchors, with a near-black visibility guard. Neutral anchors retain the original distance. This directional tolerance is provisional.'
  : value));
export const CUTOFF_METHODS = Object.freeze([...CUTOFF_QUERY_PROFILES.map(profile => Object.freeze({
  ...
  id: 'cutoff-shade-all-levels', label: 'All cutoffs: shade-aware',
  description: 'The same five-cutoff ranking with separately indexed shade-aware color tolerance. Gives darker chromatic shades more credit while preserving neutral lightness matching.',
  profile: 'all-levels', metric: 'shade-aware', family: 'cutoff-quality', representation: 'shade-coverage-quality-1024', searchKind: 'cutoff',
  approximate: false, limitations: SHADE_LIMITATIONS,
}), Object.freeze({
  id: 'cutoff-shade-hue-all-levels', label: 'All cutoffs: shade-aware + strict hue',
  description: 'Retains shade tolerance while requiring closer hue: fully chromatic anchors give full hue credit within 10° in OKLab, fading smoothly to zero at 30°. Near-neutral targets blend toward the original metric. All five cutoff scores remain independently weighted.',
  profile: 'all-levels', metric: 'shade-hue-aware', family: 'cutoff-quality', representation: 'shade-hue-coverage-quality-1024', searchKind: 'cutoff',
  approximate: false, limitations: Object.freeze([...SHADE_LIMITATIONS,
    'A separately indexed hue gate multiplies shade quality: for fully chromatic anchors, full within 10° of OKLab hue, smooth fade to zero at 30°. Near-neutral anchors blend the gate toward 1; neutral anchors retain their original metric. These angles are not HSV/HSL degrees. Fixed experimental tolerances are not population-calibrated.',
  ]),
})]);
...
export function supportsCutoff(input, query, options = {}) {
  ...
  const compiled = interpretQuery(query), warnings = [...method.limitations];
