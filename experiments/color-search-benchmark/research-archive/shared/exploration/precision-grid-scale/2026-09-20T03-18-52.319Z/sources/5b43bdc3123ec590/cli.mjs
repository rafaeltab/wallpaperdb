...
try {
  if (command === 'index') console.log(JSON.stringify(await prepareRealIndex(option('--index', INDEX)), null, 2));
    ...
    let config = { schemaVersion: 1,
      ...
      candidates: methods.map(m => ({ id: m.id, method: m.id, label: m.label, module: './adapter.mjs', ...(m.engine === 'clickhouse' ? {} : { index: m.searchKind === 'precision-grid' ? indexForMethod(m, option('--index', INDEX)) : option('--index', INDEX) }) })),
    };
