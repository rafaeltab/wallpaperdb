...
import { startVariantGenerator } from '../src/server.js';
...
function config(port: number): Config {
  ...
  return {
    ...
    otelServiceName: 'variant-generator-server-contract',
  };
}
...
it('serves readiness through a real listener and releases the listener and broker on scope exit', async () => {
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        const server = yield* startVariantGenerator(config(0));
...
it('releases acquired broker resources when its HTTP port is already occupied', async () => {
  ...
  try {
    ...
    const result = await Effect.runPromise(
      startVariantGenerator(config(address.port)).pipe(Effect.scoped, Effect.result)
    );
