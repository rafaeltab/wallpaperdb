// Isolated base512 precision-refinement indexing only. Source defaults off.
...
import { FAVORITE_UTILITY_PRESETS, createFavoriteUtilityPlan } from './favorite-utilities.mjs';
import { FAVORITE_PRECISION_DEFINITION, favoritePrecisionMapping, toFavoritePrecisionDocument, favoritePrecisionEncodingValueCount } from './favorite-precision-utilities.mjs';
...
export function favoritePrecisionIndexConfiguration(args = []) {
  const values = {};
  ...
  return { mode, scope, count, index, directory, base: url.origin, encodings: chosen, presets,
    source: values.source === 'true', seed: 99539473,
    dryRun: values.dryRun ?? false, samples, bulkBytes: 5 * 1024 * 1024 };
}

export function precisionDocumentForOrdinal({ ordinal, config, inputs, plan }) {
  ...
  return toFavoritePrecisionDocument(measurement, plan, { encodings: config.encodings });
}
...
async function prepare(config) {
  ...
  const mapping = favoritePrecisionMapping(plan, { source: config.source, encodings: config.encodings });
  const identity = { refinementDefinition: FAVORITE_PRECISION_DEFINITION, experiment: 'strict-hue-favorite-precision-utilities', version: 1, mode: config.mode, scope: config.scope, count: config.count,
...
export async function dryRunFavoritePrecisionIndex(config) {
  ...
  for (let ordinal = 0; ordinal < Math.min(config.samples, config.count); ordinal++) {
    const document = precisionDocumentForOrdinal({ ordinal, config, inputs, plan });
    const bytes = Buffer.byteLength(JSON.stringify(document));
    for (const encoding of config.encodings) totals[encoding] += favoritePrecisionEncodingValueCount(document, encoding);
...
export async function runFavoritePrecisionIndex(config) {
  ...
  try {
    ...
    for (let ordinal = 0; ordinal < config.count; ordinal++) {
      ...
      for (const encoding of config.encodings) receipt.encodingValueCounts[encoding] += favoritePrecisionEncodingValueCount(document, encoding);
    ...
    if (config.source) {
      ...
      const expected = ordinals.map(ordinal => precisionDocumentForOrdinal({ ordinal, config, inputs, plan }));
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const config = favoritePrecisionIndexConfiguration(process.argv.slice(2));
  const result = config.dryRun ? await dryRunFavoritePrecisionIndex(config) : await runFavoritePrecisionIndex(config);
