...
const s3 = await new GenericContainer("chrislusf/seaweedfs:4.47")
  .withCommand(["mini", "-s3.port=9000"])
  .withExposedPorts(9000)
  .start();
// Configure credentials, wait for storage readiness, and create buckets.
...
createTesterBuilder()
  ...
  .with(S3TesterBuilder)       // 3. Start SeaweedFS
  .build();

// Setup order:    Docker → Postgres → SeaweedFS
// Destroy order:  SeaweedFS → Postgres → Docker  (LIFO!)
```
...
Manages SeaweedFS (S3-compatible storage) containers.
