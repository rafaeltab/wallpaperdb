...
} from './profile-picture-processing.js';
...
export class ProfilePictureImportService {
  constructor(
    ...
    @inject(delay(() => ProfilePictureIngestionService))
    private readonly ingestion: ProfilePictureIngestionService,
  ...
  async importPending(isStopping: () => boolean = () => false): Promise<void> {
    ...
    const jobs = await db.query.profilePictureImports.findMany({
      where: due,
    ...
    for (const candidate of jobs) {
      ...
      const job = await db.transaction(async (tx) => {
        const [current] = await tx
          ...
          .where(and(eq(profilePictureImports.profileId, candidate.profileId), due))
        ...
        await tx
          ...
          .set({
            ...
            leaseToken,
            leaseUntil: new Date(Date.now() + this.config.profilePictureImportTimeoutMs + 60_000),
          })
          .where(eq(profilePictureImports.profileId, current.profileId));
        return { ...current, leaseToken };
      });
      if (!job?.sourceUrl) continue;
      try {
        const bytes = await downloadInitialPicture(job.sourceUrl, {
          maxBytes: this.config.profilePictureMaxBytes,
          timeoutMs: this.config.profilePictureImportTimeoutMs,
          allowedHosts: this.config.profilePictureImportHosts,
        });
        ...
        const permanent =
          error instanceof PermanentPictureImportError ||
