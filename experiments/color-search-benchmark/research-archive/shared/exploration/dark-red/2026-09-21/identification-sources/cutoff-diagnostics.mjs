...
export async function inspectCutoff({ method = 'cutoff-hard', id, query, index, signal, parameters: supplied = {} }) {
  ...
  index ??= cutoffIndexForCount(parameters.bucketCount);
  ...
  return {
    ...
    definition: cutoffQueryDefinition({ bucketCount: parameters.bucketCount, profile, cutoff: parameters.pixelCutoff, cutoffBlendExponent: parameters.cutoffBlendExponent }),
