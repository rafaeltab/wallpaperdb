...
export const METHODS = Object.freeze([...BASE_METHODS, BOUNDED_METHOD, ...FAST_METHODS, TRANSPORT_METHOD, ...REFINEMENTS, ...DIRECT_PALETTE_METHODS, ...RANK_FEATURE_METHODS, RELATIVE_METHOD, ...WIDER_ANN,PALETTE_BOUNDED_METHOD,INDEXED_HYBRID,...PRECISION_TYPED_METHODS,INDEXED_TYPED_HYBRID,...PRECISION_PRECOMPUTED_METHODS,INDEXED_PRECOMPUTED_HYBRID,...NATIVE_REFINED_METHODS,CLICKHOUSE_METHOD,...PRECISION_GRID_METHODS,...OVERLAP_METHODS,...CUTOFF_METHODS]);
export function indexForMethod(input,index=INDEX,parameters={}){const method=getMethod(input);return index===INDEX?(method.searchKind==='cutoff'?cutoffIndexForCount(cutoffParameters(method,parameters).bucketCount):method.searchKind==='overlap'?overlapIndexForCount(overlapParameters(method,parameters).bucketCount):method.searchKind==='precision-grid'?PRECISION_GRID_INDEX:method.family==='rank-features'?RANK_FEATURE_INDEX:index):index;}
