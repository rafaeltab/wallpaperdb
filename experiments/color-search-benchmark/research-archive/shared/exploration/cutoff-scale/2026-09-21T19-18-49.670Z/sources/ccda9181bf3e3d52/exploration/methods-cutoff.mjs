// THROWAWAY PROTOTYPE. Native OpenSearch globally scores indexed cutoff measurements.
...
import { CUTOFF_LEVELS, CUTOFF_PROFILES, cutoffComponents, cutoffFields } from './cutoff-definition.mjs';
...
const LIMITATIONS = Object.freeze([
  ...
  'Native OpenSearch functions rank globally. Small-corpus timings do not establish million-wallpaper latency.',
]);
export const CUTOFF_METHODS = Object.freeze(CUTOFF_PROFILES.map(profile => Object.freeze({
...
export function cutoffParameters(input, supplied = {}) {
  ...
  const { pixelCutoff = .5, namedMode = 'concrete-swatches', ...scoring } = supplied;
  ...
  return { ...overlapParameters('overlap-quality-dense', scoring), pixelCutoff, namedMode };
}

export function cutoffTargetComponents(region, profile, pixelCutoff) {
  return cutoffComponents(profile, pixelCutoff).map((component, componentIndex) => {
...
export function supportsCutoff(input, query, options = {}) {
  ...
  const targets = compiled.targets.map((target, targetIndex) => {
    ...
    const components = region ? cutoffTargetComponents(region, method.profile, parameters.pixelCutoff) : [{
