...
try {
  if (command === 'index') console.log(JSON.stringify(await prepareRealIndex(option('--index', INDEX)), null, 2));
    ...
    let dataset;
    if (args.includes('--extra-cases') || config.extraImageFiles !== undefined) {
      let extraImageFiles = config.extraImageFiles;
      if (args.includes('--extra-cases')) {
        const files = option('--extra-cases');
        if (!files || files.startsWith('--') || files.split(',').some(file => !file.trim())) throw Error('--extra-cases requires comma-separated experiment-root-relative image case paths.');
        extraImageFiles = files.split(',').map(file => file.trim());
      }
      if (!Array.isArray(extraImageFiles) || extraImageFiles.some(file => typeof file !== 'string' || !file.trim() || path.isAbsolute(file) || file.split(/[\\/]/).includes('..'))) throw Error('--extra-cases paths must stay relative to the color-search-benchmark experiment root.');
      const { loadDataset } = await import('../evaluation/loop/dataset.mjs');
      dataset = await loadDataset({ extraImageFiles });
      config.extraImageFiles = extraImageFiles;
    }
    ...
    const { run, directory } = await executeRun({ config, configDirectory: sourceDirectory, ...(dataset ? { dataset } : {}), corpus: await loadExpandedCorpus(), onProgress: console.log });
  ...
  else throw Error('Usage: cli.mjs index | evaluate [--methods IDs --repeats N --concurrency N --label TEXT --extra-cases evaluation/case.json] | browser');
