...
> {
  ...
  addMethods<TBase extends AddMethodsType<[OpenSearchTesterBuilder]>>(Base: TBase) {
    ...
    return class extends Base {
      ...
      withGatewayEnvironment() {
        this.addSetupHook(async () => {
          ...
          const opensearch: OpenSearchConfig | undefined = this.opensearch.tryGetConfig();

          if (!opensearch) {
            throw new Error('InProcessGatewayTesterBuilder requires OpenSearchTesterBuilder');
          }
          ...
          process.env.NATS_URL = 'nats://localhost:4222'; // Placeholder for now
          process.env.NATS_STREAM = 'WALLPAPER';
