...
import { OVERLAP_METHODS, supportsOverlap, buildOverlapQuery } from './methods-overlap.mjs';
...
export const METHODS = Object.freeze([...BASE_METHODS, BOUNDED_METHOD, ...FAST_METHODS, TRANSPORT_METHOD, ...REFINEMENTS, ...DIRECT_PALETTE_METHODS, ...RANK_FEATURE_METHODS, RELATIVE_METHOD, ...WIDER_ANN,PALETTE_BOUNDED_METHOD,INDEXED_HYBRID,...PRECISION_TYPED_METHODS,INDEXED_TYPED_HYBRID,...PRECISION_PRECOMPUTED_METHODS,INDEXED_PRECOMPUTED_HYBRID,...NATIVE_REFINED_METHODS,CLICKHOUSE_METHOD,...PRECISION_GRID_METHODS,...OVERLAP_METHODS]);
export function indexForMethod(input,index=INDEX){const method=getMethod(input);return index===INDEX?(method.searchKind==='overlap'?OVERLAP_INDEX:method.searchKind==='precision-grid'?PRECISION_GRID_INDEX:method.family==='rank-features'?RANK_FEATURE_INDEX:index):index;}
...
export async function executeSearch({index,seedIndex,table,method:input,query,limit=20,eligibleIds,excludedIds,filter,parameters={},signal,timeoutMs=10000,serviceTimeout}) {
  ...
  const body=method.searchKind==='overlap'?buildOverlapQuery(request):method.searchKind==='precision-grid'?buildPrecisionGridQuery(request):method.searchKind==='native-refined'?buildNativeRefinedQuery(request):method.searchKind==='precision-precomputed'?buildPrecisionPrecomputedQuery(request):method.searchKind==='precision-typed'?buildPrecisionTypedQuery(request):method.searchKind==='relative'?buildRelativeQuery(request):method.family==='rank-features'?buildRankFeatureQuery(request):method.searchKind==='direct-palette'?buildDirectPaletteQuery(request):method.searchKind==='transport'?buildTransportQuery(request):method.baseMethod?buildFastQuery(request):buildQuery(request);
